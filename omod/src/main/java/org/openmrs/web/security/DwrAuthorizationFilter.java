 ...
 * method to a {@link RequirePrivilege} annotation on the corresponding Java method. Methods without
 * an annotation are rejected at request time with {@code 403 Forbidden}; a warning is logged at
 * startup so any new DWR endpoint that ships without an annotation is visible.
 * <p>
 * For each request:
 * <ol>
 ...
 * <li>Look up {@code Script}.{method}}; if no annotation is registered (either because the pair is
 * unknown or because the method is exposed in {@code config.xml} without {@link RequirePrivilege}),
 * reject with {@code 403 Forbidden}.</li>
 ...
 * uses, because DWR clients are AJAX callers that handle authorization failures locally rather than
 * following redirects.
 */
public class DwrAuthorizationFilter implements Filter {
	...
	/**
	 * Map keyed by {@code scriptName}.{methodName}}. Value is the resolved {@link RequirePrivilege}
	 * for that method. Methods declared in {@code config.xml} but lacking an annotation are present
	 * in {@link #unannotatedMethods}; lookups fall back to authentication-only for those.
	 */
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException,
		...
		if (userContext == null || !userContext.isAuthenticated()) {
			deny(httpResponse, HttpServletResponse.SC_UNAUTHORIZED, "Authentication required to invoke " + scriptMethod);
			return;
		}
		...
		if (annotation == null) {
			...
			if (unannotatedMethods.containsKey(scriptMethod)) {
				log.warn("DWR method " + scriptMethod + " is declared in config.xml but has no @RequirePrivilege; rejecting");
			}
		...
		if (!hasRequiredPrivileges(userContext, annotation)) {
			log.warn("User '" + userContext.getAuthenticatedUser() + "' attempted to invoke DWR method " + scriptMethod
			        + " without required privilege(s): " + String.join(",", annotation.value()));
			deny(httpResponse, HttpServletResponse.SC_FORBIDDEN, "Insufficient privileges for " + scriptMethod);
			return;
		}
	 ...
	 * transport names ({@code plaincall}, {@code plainjs}, {@code htmlfile}, ...) earlier in the
	 * path. Anything that doesn't match is a static asset or a JS-interface generation request and
	 * gets returned as {@code null} so the caller knows to skip enforcement.
	 */
	...
	/**
	 * Parses {@code config.xml} from the legacyui classpath and resolves each declared DWR method
	 * to its {@link RequirePrivilege} annotation. Methods without the annotation are collected
	 * separately so the filter can warn at startup rather than fail outright while the rollout is
	 * in progress.
	 */
	...
	/**
	 * Finds the first method on {@code dwrClass} (or any superclass) whose name matches and returns
	 * its {@link RequirePrivilege}, if any. DWR resolves overloads by argument count at runtime;
	 * for our purposes we treat all overloads of the same name as sharing one privilege
	 * requirement, which matches every real-world legacyui DWR script.
	 */
