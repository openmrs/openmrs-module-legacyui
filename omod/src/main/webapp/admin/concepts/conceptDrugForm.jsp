...
<openmrs:extensionPoint pointId="org.openmrs.admin.concepts.conceptDrugForm.afterTitle" type="html"
                        parameters="drugId=${drug.drugId}"/>
...
<form method="post">
    <fieldset>
        <table cellpadding="3" cellspacing="0" id="table">
            ...
            <tr>
                ...
                <td>
                    <spring:bind path="drug.concept">
                        <openmrs_tag:conceptField formFieldName="${status.expression}" formFieldId="concept"
                                                  initialValue="${status.value}" includeClasses="Drug"/>
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <spring:bind path="drug.dosageForm">
                        <openmrs_tag:conceptField formFieldName="${status.expression}" formFieldId="dosageForm"
                                                  initialValue="${status.value}"
                                                  includeClasses="${dosageFormConceptClasses}"/>
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
                </td>
            </tr>
                ...
                <td>
                    <table cellpadding="3" cellspacing="0">
                        <c:forEach var="ingredient" items="${drug.ingredients}">
                            <tr>
                                <td><openmrs:format concept="${ingredient.ingredient}"/> - ${ingredient.quantity}
                                    <openmrs:format concept="${ingredient.units}"/></td>
                            </tr>
                        </c:forEach>
                    </table>
                </td>
            ...
            <c:if test="${drug.creator != null}">
                <tr>
                    ...
                    <td>
                        <a href="#View User" onclick="return gotoUser(null, '${drug.creator.userId}')"><c:out
                                value="${drug.creator.personName}"/></a> -
            ...
            <c:if test="${drug.changedBy != null}">
                <tr>
                    ...
                    <td>
                        <a href="#View User" onclick="return gotoUser(null, '${drug.changedBy.userId}')"><c:out
                                value="${drug.changedBy.personName}"/></a>
        ...
        <openmrs:extensionPoint pointId="org.openmrs.admin.concepts.conceptDrugForm.inForm" type="html"
                                parameters="drugId=${drug.drugId}"/>
        ...
        <input type="button" value='<openmrs:message code="general.cancel"/>'
               onclick="history.go(-1); return; document.location='index.htm?autoJump=false&phrase=<c:out
                       value="${param.phrase}"/>'">
    </fieldset>
</form>
<br/>
...
<openmrs:extensionPoint pointId="org.openmrs.admin.concepts.conceptDrugForm.footer" type="html"
                        parameters="drugId=${drug.drugId}"/>
