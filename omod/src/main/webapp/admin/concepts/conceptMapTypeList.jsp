...
<openmrs:require privilege="Manage Concept Map Types" otherwise="/login.htm"
                 redirect="/admin/concepts/conceptMapTypeList.list"/>
    ...
    <table cellpadding="3" cellspacing="3">
        ...
        <c:forEach var="conceptMapType" items="${conceptMapTypeList}">
            <tr <c:if test="${conceptMapType.isHidden == true }">style='color:red'</c:if>>
                ...
                <td>&nbsp;</td>
                <td valign="top" <c:if test="${conceptMapType.isHidden == true }">style='color:grey'</c:if>><c:out
                        value="${conceptMapType.description}"/></td>
            </tr>
        </c:forEach>
    </table>
</div>
