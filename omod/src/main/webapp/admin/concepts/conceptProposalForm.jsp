...
<script type="text/javascript">
    ...
    var selectConceptId = function (id) {
        var closure = function (thisObj, method) {
            return function (obj) {
                return thisObj[method]({"obj": obj});
            };
        };
...
<form method="post">
    <table>
        ...
        <tr>
            ...
            <td>
                <spring:bind path="conceptProposal.finalText">
                    <input type="text" name="${status.expression}" id="finalText"
                           value="<c:if test="${(status.value == null || status.value == '') && conceptProposal.mappedConcept == null}">${conceptProposal.originalText} </c:if><c:if test="${status.value != ''}">${status.value}</c:if>"
                           size="50"/>
        ...
        <tr>
            ...
            <td>
                <spring:bind path="conceptProposal.mappedConcept">
                    <a target="_blank" href="${pageContext.request.contextPath}/dictionary/concept.form?conceptName="
                       onclick="this.href=this.href + document.getElementById('finalText').value"><openmrs:message
                            code="Concept.add"/></a>
                    <openmrs:message code="general.or"/>
                    <br/>
                    <div dojoType="ConceptSearch" widgetId="cSearch" showVerboseListing="true"></div>
                    <div dojoType="OpenmrsPopup" widgetId="cSelection" hiddenInputName="conceptId" searchWidget="cSearch"
        ...
        <tr>
            ...
            <td>
                <input type="radio" name="actionToTake" value="saveAsSynonym" checked="checked"> <openmrs:message
                    code="ConceptProposal.saveAsSynonym"/>
                <input type="radio" name="actionToTake" value="saveAsMapped"> <openmrs:message
                    code="ConceptProposal.saveAsMapped"/>
            </td>
        </tr>
    </table>
    <br/>
