...
<c:if test="${conceptReferenceTermModel.conceptReferenceTerm.conceptReferenceTermId != null && 
    ...
    <form:form action="unretireConceptReferenceTerm.htm" method="post" modelAttribute="conceptReferenceTermModel">
        ...
        <div class="voidedMessage">
            <div>
                <openmrs:message code="ConceptReferenceTerm.retiredMessage"/> &nbsp;<input type="submit"
                                                                                           value='<openmrs:message code="general.restore"/>'/>
            </div>
        </div>
    </form:form>
</c:if>
...
<form:form method="post" action="conceptReferenceTerm.form" modelAttribute="conceptReferenceTermModel">
    ...
    <fieldset>
        ...
        <table cellpadding="3" cellspacing="3" class="form_align_left">
                            ...
                            <select name="${status.expression}">
                                ...
                                <openmrs:forEachRecord name="conceptSource">
                                    <option value="${record.conceptSourceId}"
                                            <c:if test="${record.conceptSourceId == status.value}">selected="selected"</c:if>>
                                        <c:out value="${record.name}"/> <c:if test="${not empty record.hl7Code}">[<c:out
                                            value="${record.hl7Code}"/>]</c:if>
                                    </option>
            ...
            <tr>
                <th class="alignRight" valign="top" style="padding-top: 8px"><openmrs:message
                        code="ConceptReferenceTerm.relatedTerms"/></th>
                <td valign="top">
                            ...
                            <spring:nestedPath path="termMaps[${mapStatus.index}]">
                                <tr <c:if test="${mapStatus.index % 2 == 0}">class='evenRow'</c:if>>
                                        ...
                                        <c:otherwise>
                                            <td>
                                                <select id="term[${mapStatus.index}].source">
                                                    ...
                                                    <openmrs:forEachRecord name="conceptSource">
                                                        <option value="${record.conceptSourceId}"
                                                                <c:if test="${record.conceptSourceId == map.termB.conceptSource.conceptSourceId}">selected="selected"</c:if>>
                                                            <c:out value="${record.name}"/>
                                                        </option>
                                                    </openmrs:forEachRecord>
                                                </select>
                                            ...
                                            <td>
                                                <spring:bind path="termB">
                                                    <openmrs_tag:conceptReferenceTermField
                                                            formFieldName="${status.expression}"
                                                            formFieldId="${status.expression}"
                                    ...
                                    <td>
                                        ...
                                        <input type="button" value='<openmrs:message code="general.remove"/>'
                                               class="smallButton"
                                               onClick="javascript:this.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode)"/>
                                        <spring:bind path="conceptReferenceTermModel.termMaps[${mapStatus.index}]"
            ...
            <c:if test="${fn:length(referenceTermMappingsToThisTerm) > 0}">
                <tr>
                    <th class="alignRight" valign="top"><openmrs:message
                            code="ConceptReferenceTerm.termsWithMappingsToThis"/></th>
                    <td valign="top">
            ...
            <c:if test="${conceptReferenceTermModel.conceptReferenceTerm.creator != null}">
                <tr>
                    ...
                    <td>
                        ...
                        <openmrs:formatDate date="${conceptReferenceTermModel.conceptReferenceTerm.dateCreated}"
                                            type="long"/>
                    </td>
                </tr>
            </c:if>
            <c:if test="${conceptReferenceTermModel.conceptReferenceTerm.changedBy != null}">
                <tr>
                    ...
                    <td>
                        ...
                        <openmrs:formatDate date="${conceptReferenceTermModel.conceptReferenceTerm.dateChanged}"
                                            type="long"/>
                    </td>
                </tr>
            </c:if>
            <c:if test="${conceptReferenceTermModel.conceptReferenceTerm.retiredBy != null}">
                <tr>
                    ...
                    <td>
                        ...
                        <openmrs:formatDate date="${conceptReferenceTermModel.conceptReferenceTerm.dateRetired}"
                                            type="long"/>
                    </td>
                </tr>
            </c:if>
            ...
            <c:if test="${conceptReferenceTerm.conceptReferenceTermId != null}">
                <tr>
                    <th class="alignRight"><font color="#D0D0D0"><sub><openmrs:message code="general.uuid"/></sub></font>
                    </th>
                    <td colspan="${fn:length(locales)}"><font color="#D0D0D0"><sub>${conceptReferenceTerm.uuid}</sub></font>
                    </td>
                </tr>
            </c:if>
            <tr>
                ...
                <td>
                    <table cellpadding="0" cellspacing="20" align="left">
                        <tr>
                            ...
                            <td><input type="button" value="<openmrs:message code="general.cancel"/>"
                                       onclick="javascript:window.location='conceptReferenceTerms.htm'"></td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <tr>
            <td>
                <c:if test="${conceptReferenceTermModel.conceptReferenceTerm.retired == false }">
                    <input type="submit" value='<openmrs:message code="general.retire"/>'
                           onclick="javascript:$j('#retire-dialog').dialog('open')"/>
                    <div id="retire-dialog" title="<openmrs:message code="general.retire.confirmation"/>">
                        <form:form action="retireConceptReferenceTerm.htm" method="post"
                            ...
                            <table cellpadding="3" cellspacing="3" align="center">
                                ...
                                <tr>
                                    <td colspan="2" style="text-align: center">
                                        <input type="submit" value="<openmrs:message code="general.retire"/>"/> &nbsp; <input
            ...
            <td>
                <openmrs:hasPrivilege privilege="Purge Concept Reference Terms">
                    <input type="button" value='<openmrs:message code="general.purge"/>'
                           onclick="javascript:$j('#purge-dialog').dialog('open')"/>
                    <div id="purge-dialog" title="<openmrs:message code="general.purge.confirmation"/>">
                        <form:form action="purgeConceptReferenceTerm.htm" method="post"
                                   modelAttribute="conceptReferenceTermModel">
                            <input type="hidden" name="conceptReferenceTermId" value="${param.conceptReferenceTermId}"/>
                            <br/><br/>
                            ...
                            <table cellpadding="3" cellspacing="30" align="center">
                                <tr>
                                    <td>
                                        <input type="submit" value='<openmrs:message code="general.yes"/>'/> &nbsp; <input
                                            type="button" value="<openmrs:message code="general.no"/>"
