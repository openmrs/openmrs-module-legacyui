...
<c:if test="${encounter.patient.patientId != null}">
    <a href="../../patientDashboard.form?patientId=<c:out value="${encounter.patient.patientId}" />"><openmrs:message
...
<form method="post" onsubmit="return onFormSubmission()">
        ...
        <table cellpadding="3" cellspacing="0">
                    ...
                    <div id="patient_editable">
                        <spring:bind path="encounter.patient">
                            <openmrs_tag:patientField formFieldName="patientId" searchLabelCode="Patient.find"
                                                      initialValue="${status.value.patientId}"
                                                      linkUrl="${pageContext.request.contextPath}/admin/patients/patient.form"
                                                      callback="updateSaveButtonAndVisits"
                                                      allowSearch="${encounter.encounterId == null}"/>
                        ...
                        <select id="visitSelect" name="${status.expression}">
                            ...
                            <c:forEach items="${patientVisits}" var="visit">
                                <option value="${visit.visitId}"
                                        <c:if test="${visit.visitId == status.value}">selected="selected"</c:if>>
                                    <openmrs:formatDate date="${visit.startDatetime}"/>
                                    <c:out value="${visit.visitType.name}"/> <c:out value="${visit.patient.personName}"/>
                                    <c:if test="${visit.indication != null}"> <c:out
                                            value="${visit.indication.name}"/></c:if>
                                ...
                                <select name="encounterType">
                                    ...
                                    <optgroup label="<openmrs:message code="Encounter.type.retired"/>">
                                        ...
                                        <option value="${type.encounterTypeId}"
                                                <c:if test="${type.encounterTypeId == status.value}">selected</c:if>><c:out
                                ...
                                <select name="form">
                                    ...
                                    <c:forEach items="${forms}" var="form">
                                        <option value="${form.formId}"
                                                <c:if test="${form.formId == status.value}">selected</c:if>><c:out
                                                value="${form.name}"/></option>
                                    </c:forEach>
                                </select>
                            </c:when>
                            <c:otherwise>
            ...
            <c:if test="${encounter.encounterId != null}">
                <tr>
                    ...
                    <td>
                        <a href="#View User" onclick="return gotoUser(null, '${encounter.creator.userId}')"><c:out
                                value="${encounter.creator.personName}"/></a> -
                ...
                <tr>
                    ...
                    <td>
                        <spring:bind path="encounter.voided">
                            ...
                            <input type="checkbox" name="${status.expression}" id="voided" onClick="voidedClicked(this)"
                                   <c:if test="${encounter.voided}">checked</c:if> />
                            <c:if test="${status.errorMessage != ''}"><span
                                    class="error">${status.errorMessage}</span></c:if>
                        </spring:bind>
                    </td>
                ...
                <c:if test="${encounter.voidedBy != null}">
                    <tr id="voidedBy">
                        ...
                        <td>
                            <a href="#View User" onclick="return gotoUser(null, '${encounter.voidedBy.userId}')"><c:out
                                    value="${encounter.voidedBy.personName}"/></a> -
            ...
            <c:forEach items="${encounter.providersByRoles}" var="providerRole">
                <c:forEach items="${providerRole.value}" var="provider">
                    <tr>
                        ...
                        <td><input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                                   onClick="removeProvider(this, ${providerRole.key.encounterRoleId}, ${provider.providerId})"/>
                        </td>
                    </tr>
                </c:forEach>
            </c:forEach>
            <tr id="addNewProviderTemplate" style="display:none;">
                ...
                <td>
                    <input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                           onclick="removeProvider(this)"/>
                </td>
            </tr>
        </table>
        <input type="button" id="addProviderButton" value='<openmrs:message code="Provider.add"/>' class="smallButton"
               onclick="addProvider()"/>
    </div>
    <br/>
    ...
    <input type="button" value='<openmrs:message code="general.cancel"/>'
           onclick="history.go(-1); return; document.location='index.htm?autoJump=false&phrase=${openmrs:getSafeJsString(param.phrase)}'">
</form>
...
<c:if test="${encounter.encounterId != null}">
    <br/>
    <openmrs:extensionPoint pointId="org.openmrs.admin.encounters.encounterFormBeforeObs" type="html"
                            parameters="encounterId=${encounter.encounterId}">
            ...
            <div class="box" style="padding: 0px 0px 5px;"><openmrs:message code="${extension.content}"/>
                <c:if test="${extension.portletUrl != null}">
                    <openmrs:portlet url="${extension.portletUrl}" moduleId="${extension.moduleId}"
                                     id="${extension.portletUrl}" encounterId="${encounter.encounterId}"
                                     parameters="allowEdits=true"/>
                </c:if>
            </div>
            <br/>
    ...
    <div class="boxHeader">
		<span style="float: right">
			<a href="#" id="showDescription"
               onClick="return toggleVisibility(document, 'div', 'description')"><openmrs:message
                    code="general.toggle.description"/></a> |
			<a href="#" id="showVoided" onClick="return toggleRowVisibilityForClass('obs', 'voided', true);"><openmrs:message
                    code="general.toggle.voided"/></a>
		</span>
    ...
    <div id="encounterFormAddObsMenu">
        ...
        <openmrs:extensionPoint pointId="org.openmrs.admin.encounters.encounterFormAddObsMenu" type="html"
                                requiredClass="org.openmrs.module.web.extension.LinkProviderExtension">
