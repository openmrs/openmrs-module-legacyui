    ...
    <c:choose>
        <c:when test="${obs.obsGrouping}">
            <tr class="<c:if test="${obs.voided}">voided </c:if>obsGroupHeader">
                <td>${field.fieldNumber}<c:if
                        test="${field.fieldPart != null && field.fieldPart != ''}">.${field.fieldPart}</c:if></td>
        ...
        <c:otherwise>
            <tr class='${count % 2 == 0 ? "evenRow" : "oddRow"} ${obs.voided ? "voided" : ""}'
                ...
                <td class="obsConceptName" style="padding-left: ${padding}"><a
                        href="${pageContext.request.contextPath}/admin/observations/obs.form?obsId=${obs.obsId}"
                        onclick="return click('${obs.obsId}')"><c:out value="${obs.concept.name.name}"/></a></td>
                <td class="obsValue"><openmrs:format obsValue="${obs}"/></td>
                <td class="obsAlerts" valign="middle" align="right">
                    <c:choose>
                        <c:when test="${fn:contains(editedObs, obs.obsId)}"><span class="obsEdit"><img
                                src="${pageContext.request.contextPath}/images/alert.gif"
                                title='<openmrs:message code="Obs.edited"/>'/></span></c:when>
                        <c:otherwise><span class="obsEdit"><c:if
                                test="${obs.encounter != null && obs.dateCreated != obs.encounter.dateCreated}"><img
                                src="${pageContext.request.contextPath}/images/alertPlus.gif"
                                title='<openmrs:message code="Obs.afterEncounter.created"/>'/></c:if></span></c:otherwise>
                    </c:choose>
                    <span class="obsComment"><c:if test="${obs.comment != null && obs.comment != ''}"><img
                            src="${pageContext.request.contextPath}/images/note.gif" title="${obs.comment}"/></c:if></span>
                </td>
            ...
            <tr class='${status.index % 2 == 0 ? "evenRow" : "oddRow"} ${obs.voided ? "voided" : ""}'
                ...
                <td colspan="5" style="padding-left: ${padding}">
                    <div class="description"><c:out value="${obs.concept.description}"/></div>
                </td>
            </tr>
        </c:otherwise>
    </c:choose>
