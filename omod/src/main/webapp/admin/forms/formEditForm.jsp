...
<c:if test="${form.retired}">
    <div class="retiredMessage">
        <div><openmrs:message code="Form.retiredMessage"/></div>
    </div>
</c:if>
...
<form method="post" enctype="multipart/form-data">
        ...
        <tr>
            ...
            <td>
                <spring:bind path="form.name">
                    ...
                    <c:if test="${status.errorMessage != ''}"><c:if test="${status.errorMessage != ''}"><span
                            class="error">${status.errorMessage}</span></c:if></c:if>
                </spring:bind>
            </td>
        </tr>
        <tr>
    ...
    <c:if test="${not empty param.duplicate}">
        <input type="submit" name="action" value="<openmrs:message code="Form.create.duplicate"/>"
               <c:if test="${formsLocked == 'true'}">disabled</c:if>>
    </c:if>
    <c:if test="${empty param.duplicate}">
        ...
        <c:if test="${form.formId != null && not isBasicForm}">
            <openmrs:hasPrivilege privilege="Delete Forms">
                &nbsp; &nbsp; &nbsp;
                <input type="submit" name="action" value="<openmrs:message code="Form.delete"/>"
                       onclick="return confirm('<openmrs:message code="Form.confirmation"/>')"
                       <c:if test="${formsLocked == 'true'}">disabled</c:if>/>
...
<c:if test="${form.formId != null && empty param.duplicate}">
    ...
    <c:if test="${form.published == true}">
        <div class="retiredMessage">
            <div><openmrs:message code="Form.design.disabled"/></div>
        </div>
    </c:if>
    ...
    <table width="99%">
        <tr>
            <td valign="top">
                <div dojoType="Tree" menu="treeContextMenu" strictFolders="false" DNDMode="between" toggler="fade"
                     widgetId="tree" DNDAcceptTypes="<c:if test="${form.published != true}">*</c:if>"
                     controller="treeController" selector="treeSelector">
                </div>
            </td>
            <td valign="top" style="padding-left: 5px;" id="fieldSearch" width="40%">
                <div id="fieldSearchDiv">
                    <c:if test="${form.published != true && not isBasicForm}">
                        <div dojoType="FieldSearch" widgetId="fieldSearch"
                             searchLabel='<openmrs:message code="Field.find" />' showHeaderRow="false"
                             alsoSearchConcepts="true"></div>
                    </c:if>
                </div>
            </td>
        </tr>
    ...
    <div id="editFormField">
        ...
        <form xonsubmit="save(selectedNode)" style="padding: 0px; margin: 0px; overflow: auto">
            ...
            <c:if test="${form.published != true && not isBasicForm}">
                <input type="submit" id="saveFormField" onclick="return save(selectedNode);"
                       value="<openmrs:message code="general.save"/>"/>
            </c:if>
            <input type="button" id="cancelFormField" onclick="cancelClicked()"
