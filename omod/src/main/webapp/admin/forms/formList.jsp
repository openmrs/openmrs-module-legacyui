...
<form style="padding: 0px; margin: 0px; display: inline;">
    ...
    <select name="duplicateFormId" id="duplicateFormId" <c:if test="${formsLocked == 'true'}"> disabled </c:if>>
        ...
        <c:forEach var="form" items="${formList}">
            <option value="${form.formId}"><c:out value="${form.name}"/> <c:out
                    value="(v. ${form.version}.${form.build})"/></option>
        </c:forEach>
    </select>
    <input type="button" value="<openmrs:message code="Form.duplicate"/>" onclick="return duplicate()" <c:if
            test="${formsLocked == 'true'}"> disabled </c:if>/>
</form>
...
<div class="boxHeader">
	<span style="float: right">
		<a href="#" id="showRetired" onClick="return toggleRowVisibilityForClass('formTable', 'voided');"><openmrs:message
                code="general.toggle.retired"/></a>
	</span>
    ...
    <table cellpadding="2" cellspacing="0" id="formTable" width="98%">
        ...
        <c:forEach var="form" items="${formList}" varStatus="status">
            <tr class='${status.index % 2 == 0 ? "evenRow" : "oddRow"} ${form.retired ? "voided" : ""}'>
                <td valign="top" style="white-space: nowrap"><a href="formEdit.form?formId=${form.formId}"><c:out
                        value="${form.name}"/></a></td>
