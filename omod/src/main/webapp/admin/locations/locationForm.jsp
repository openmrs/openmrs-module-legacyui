...
<openmrs:extensionPoint pointId="org.openmrs.admin.locations.locationForm.afterTitle" type="html"
...
<form method="post">
    <fieldset>
        <table class="left-aligned-th">
            ...
            <spring:nestedPath path="location">
                <openmrs:portlet url="addressLayout" id="addressPortlet" size="full"
                                 parameters="layoutShowTable=false|layoutShowExtended=false|layoutShowErrors=false|isNew=${location.locationId == null}"/>
            </spring:nestedPath>
            <tr>
                <th valign="top"><openmrs:message code="Location.parentLocation"/></th>
                <td colspan="5">
                    <spring:bind path="location.parentLocation">
                        <openmrs_tag:locationField formFieldName="parentLocation" initialValue="${status.value}"
                                                   optionHeader="[blank]"/>
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
                </td>
            ...
            <c:forEach var="attrType" items="${ attributeTypes }">
                <c:if test="${ !attrType.retired }">
                    <openmrs_tag:attributesForType attributeType="${ attrType }" customizable="${ location }"
                        ...
                        <c:forEach var="t" items="${locationTags}">
                            ...
                            <span <c:if test="${t.retired}">class="retired"</c:if>>
                                <input type="checkbox" name="tags" value="${t.id}"
                                       <c:if test="${openmrs:collectionContains(status.value, t)}">checked="true"</c:if>/>
        ...
        <openmrs:extensionPoint pointId="org.openmrs.admin.locations.locationForm.inForm" type="html"
                                parameters="locationId=${location.locationId}"/>
        <br/>
    ...
    <div class="box">
        <form action="" method="post" onsubmit="return confirmLocationPurge()">
            <table cellpadding="3" cellspacing="0">
                ...
                <tr>
                    <td>
                        <input type="submit" value='<openmrs:message code="legacyui.Location.purgeLocation"/>'
                               name="purgeLocation">
                    </td>
                </tr>
            </table>
...
<openmrs:extensionPoint pointId="org.openmrs.admin.locations.locationForm.footer" type="html"
                        parameters="locationId=${location.locationId}"/>
