    ...
    <table cellpadding="1" cellspacing="0">
        ...
        <c:forEach var="globalProp" items="${globalProps}" varStatus="status">
            <tr class="${status.index % 2 == 0 ? 'evenRow' : 'oddRow' }">
                <td valign="top"><input type="text" name="property" value="${globalProp.property}" size="50" maxlength="250"
                                        onkeyup="edited()"/></td>
                <td valign="top">
                    <c:choose>
                        <c:when test="${fn:length(globalProp.propertyValue) > 20}">
                            <textarea name="value" onkeyup="edited()" rows="1" cols="60"
                                      wrap="off">${globalProp.propertyValue}</textarea>
                        </c:when>
                        <c:otherwise>
                            <input type="text" name="value" value="${globalProp.propertyValue}" size="30" maxlength="4000"
                                   onkeyup="edited()"/>
                        </c:otherwise>
                    </c:choose>
                </td>
                <td valign="top" rowspan="2"><input type="button" align="right"
                                                    value='<openmrs:message code="general.remove" />' class="closeButton"
                                                    onclick="edited(); removeProperty(this)"/></td>
            ...
            <tr class="${status.index % 2 == 0 ? 'evenRow' : 'oddRow' }">
                <td colspan="2" valign="top" class="description">
						<textarea name="description" class="descriptionBox"
                                  ...
                                  onfocus="descriptionFocus(this)" onblur="descriptionBlur(this)"><c:out
                                value="${globalProp.description}"/></textarea>
                </td>
            </tr>
        </c:forEach>
        <tr id="newProperty">
            ...
            <td valign="top" rowspan="2"><input type="button" value='<openmrs:message code="general.remove" />'
                                                class="closeButton" onclick="removeProperty(this)"/></td>
        </tr>
