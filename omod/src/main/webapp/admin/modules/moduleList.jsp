...
<div id="dialog-confirm" title="<openmrs:message code="Module.dependencyValidationNotice"/>"><p
        id="dependency-confirmation-message"></p></div>

<c:choose>
    <c:when test="${allowAdmin == 'true'}">
        <div id="buttonPanel">
            <div style="float:left">
                <input type="button" id="addUpgradeButton"
                       value="<openmrs:message code="Module.addOrUpgrade" javaScriptEscape="true"/>"/>
                <div id="addUpgradePopup">
                    ...
                    <div class="box">
                        <form id="moduleAddForm" action="module.list" method="post" enctype="multipart/form-data">
                            <input type="file" name="moduleFile" size="40"
                                   <c:if test="${allowAdmin!='true'}">disabled="disabled"</c:if> />
                    ...
                    <div id="findModule">
                        <b class="boxHeader"><openmrs:message code="Module.findAndDownload"
                                                              arguments="${moduleRepositoryURL}"/></b>
                        <div class="box">
            ...
            <div style="float:right">
                <form method="post"><input type="submit" name="action" value='<openmrs:message code="Module.startAll"/>'/>
                </form>
            </div>
...
<c:forEach var="module" items="${moduleList}" varStatus="varStatus">
    ...
    <form id="${module.moduleId}-form" method="post">
        ...
        <tr class='${varStatus.index % 2 == 0 ? "oddRow" : "evenRow" }' id="${module.moduleId}">
                    ...
                    <td valign="top">
                        <c:choose>
                            <c:when test="${not module.started}">
                                <input type="image" src="${pageContext.request.contextPath}/images/play.gif" name="start"
                                       onclick="document.getElementById('hiddenAction').value = this.value"
                                       title="<openmrs:message code="Module.start.help"/>"
                                       alt="<openmrs:message code="Module.start"/>"/>
                            </c:when>
                            <c:otherwise>
                                <input type="image" src="${pageContext.request.contextPath}/images/stop.gif" name="stop"
                                       onclick="return getDependencies(this, false);"
                                       title="<openmrs:message code="Module.stop.help"/>"
                                       alt="<openmrs:message code="Module.stop"/>"/>
                            </c:otherwise>
                        </c:choose>
                    </td>
                    <td valign="top"><input type="image" src="${pageContext.request.contextPath}/images/trash.gif"
                                            name="unload" onclick="return getDependencies(this, true);"
                                            title="<openmrs:message code="Module.unload.help"/>"
                                            title="<openmrs:message code="Module.unload"/>"
                                            alt="<openmrs:message code="Module.unload"/>"/></td>
                </c:when>
                <c:otherwise>
                    <td valign="top">
                        <img src="${pageContext.request.contextPath}/images/lock.gif"
                             title="<openmrs:message code="Module.locked.help"/>"
                             alt="<openmrs:message code="Module.locked"/>"/>
                    </td>
                    <td></td>
                </c:otherwise>
            </c:choose>
            <td valign="top">${module.name} <c:if test="${not module.started}"><b id="moduleNotStarted"
                                                                                  style="white-space: nowrap">[<openmrs:message
                    code="Module.notStarted"/>]</b></c:if></td>
            ...
            <td valign="top"
                <c:if test="${module.startupErrorMessage != null}">class="error"</c:if> >
