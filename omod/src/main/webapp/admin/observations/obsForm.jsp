...
<form method="post" onSubmit="removeHiddenRows()" enctype="multipart/form-data">
    ...
    <fieldset>
            ...
            <table id="obsTable">
                            ...
                            <c:choose>
                                <c:when test="${obs.encounter != null}">
                                    <input type="text" name="person" id="person"
                                           value="<c:out value="${status.editor.value.personName}"/>" size="20"
                                           disabled="disabled"/>
                                </c:when>
                                <c:otherwise>
                                    <openmrs_tag:personField formFieldName="person" searchLabelCode="Person.findBy"
                                                             initialValue="${status.editor.value.personId}" linkUrl=""
                                                             callback=""/>
                                </c:otherwise>
                            </c:choose>
                    ...
                    <td>
                        <spring:bind path="encounter">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <c:out value="${status.editor.value.location.name}"/> - <openmrs:formatDate
                                        date="${status.editor.value.encounterDatetime}" type="medium"/>
                                    <a href="${pageContext.request.contextPath}/admin/encounters/encounter.form?encounterId=${status.editor.value.encounterId}"><openmrs:message
                                            code="general.view"/>/<openmrs:message code="general.edit"/></a>
                                </c:otherwise>
                            </c:choose>
                        </spring:bind>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <spring:bind path="order">
                            <input type="text" name="order" id="order" value="${status.editor.value.orderId}" size="7"
                ...
                <tr>
                    ...
                    <td>
                        <spring:bind path="obs.concept">
                            <openmrs_tag:conceptField formFieldName="concept" formFieldId="conceptId" excludeDatatypes="N/A"
                ...
                <c:if test="${1 == 2}">
                    <tr>
                        ...
                        <td>
                            <spring:bind path="accessionNumber">
                                <input type="text" name="${status.expression}" id="accessionNumber" value="${status.value}"
                                       size="10"
                                       <c:if test="${obs.obsId != null}">disabled</c:if> />
                                <c:if test="${status.errorMessage != ''}"><span
                                        class="error">${status.errorMessage}</span></c:if>
                    ...
                    <tr>
                        ...
                        <spring:bind path="valueGroupId">
                            <td>
                                <input type="text" name="${status.expression}" id="valueGroupId" value="${status.value}"
                                       size="10"
                                       <c:if test="${obs.obsId != null}">disabled</c:if> />
                                <c:if test="${status.errorMessage != ''}"><span
                                        class="error">${status.errorMessage}</span></c:if>
                    ...
                    <spring:bind path="valueBoolean">
                        <td>
                            <select name="${status.expression}" id="valueBooleanSelect">
                                <option value=""
                                        <c:if test="${status.value == null || status.value == ''}">selected</c:if>></option>
                                <option value="true" <c:if test="${status.value == 'true'}">selected</c:if>><openmrs:message
                                        code="general.true"/></option>
                                <option value="false" <c:if test="${status.value == 'false'}">selected</c:if>>
                                    <openmrs:message code="general.false"/></option>
                            </select>
                            <c:if test="${status.errorMessage != ''}"><span
                                    class="error">${status.errorMessage}</span></c:if>
                        </td>
                ...
                <tr id="valueCodedRow" class="obsValue">
                    ...
                    <td>
                        <spring:bind path="valueCoded">
                            <openmrs_tag:conceptField formFieldName="valueCoded" formFieldId="valueCoded"
                                                      initialValue="${status.editor.value.conceptId}"
                                                      showAnswers="${obs.concept.conceptId}"
                                                      onSelectFunction="onAnswerSelect"/>
                ...
                <tr id="valueDatetimeRow" class="obsValue">
                    ...
                    <td>
                        <spring:bind path="valueDatetime">
                            ...
                            (<openmrs:message code="general.format"/>: <openmrs:datePattern/> <openmrs:timePattern
                                format="jquery"/>)
                            <c:if test="${status.errorMessage != ''}"><span
                                    class="error">${status.errorMessage}</span></c:if>
                        </spring:bind>
                    ...
                    <spring:bind path="valueNumeric">
                        <td>
                            <input type="text" name="${status.expression}" value="${status.value}" size="10"
                                   onKeyUp="validateNumericRange(this.value)"/>
                ...
                <tr id="valueModifierRow" class="obsValue">
                    ...
                    <spring:bind path="valueModifier">
                        <td>
                            <input type="text" name="${status.expression}" id="valueModifierInput" value="${status.value}"
                                   size="3" maxlength="2"/>
                            <c:if test="${status.errorMessage != ''}"><span
                                    class="error">${status.errorMessage}</span></c:if>
                        </td>
                    ...
                    <spring:bind path="valueComplex">
                        <td>
                            ...
                            <a href="${hyperlinkView}" target="_blank"><openmrs:message
                                    code="Obs.viewCurrentComplexValue"/></a><br/>
                            <a href="${hyperlinkView}&download" target="_blank"><openmrs:message
                                    code="Obs.downloadCurrentComplexValue"/></a><br/>
                ...
                <openmrs:extensionPoint pointId="org.openmrs.admin.observations.belowValueRow" type="html"
                                        parameters="obsId=${obs.obsId}"></openmrs:extensionPoint>
        ...
        <c:if test="${obs.obsId != null}">
            <b><openmrs:message code="Obs.edit.reason"/></b> <input type="text" value="${editReason}" size="40"
                                                                    name="editReason"/>
            <spring:hasBindErrors name="obs">
                <c:forEach items="${errors.allErrors}" var="error">
                    <c:if test="${error.code == 'editReason'}"><span class="error"><openmrs:message
        ...
        <input type="submit" name="saveObs" value='<openmrs:message code="Obs.save"/>'
               <c:if test="${obs.voided}">disabled</c:if> >
    ...
    <c:if test="${not obs.voided && not empty obs.obsId}">
        <form action="" method="post">
            <fieldset>
                ...
                <spring:hasBindErrors name="obs">
                    <c:forEach items="${errors.allErrors}" var="error">
                        <c:if test="${error.code == 'voidReason'}"><span class="error"><openmrs:message
                                code="${error.defaultMessage}" text="${error.defaultMessage}"/></span></c:if>
                    </c:forEach>
                </spring:hasBindErrors>
                <br/>
...
<c:if test="${obs.obsId != null}">
    <br/>
    <openmrs:extensionPoint pointId="org.openmrs.admin.observations.obsFormBottom" type="html"
                            parameters="obsId=${obs.obsId}">
            ...
            <div class="box" style="padding: 0px 0px 5px;"><openmrs:message code="${extension.content}"/>
                <c:if test="${extension.portletUrl != null}">
                    <openmrs:portlet url="${extension.portletUrl}" moduleId="${extension.moduleId}"
                                     id="${extension.portletUrl}" parameters="allowEdits=true|obsId=${obs.obsId}"/>
                </c:if>
            </div>
            <br/>
        </openmrs:hasPrivilege>
