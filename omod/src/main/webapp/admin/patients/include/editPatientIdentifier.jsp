    ...
    <tr>
        ...
        <td>
            <spring:bind path="preferred">
                ...
                <input type="checkbox" name="${status.expression}" onclick="if (preferredBoxClick) preferredBoxClick(this)"
                       value="true" alt="patientIdentifier"
        ...
        <td>
            <spring:bind path="identifier">
                <input type="text"
                       ...
                       value="${status.value}"
                       onKeyUp="return true; validateIdentifier(this, 'saveButton', '<openmrs:message
                               code="error.identifier"/>');"
                       onChange="return true; validateIdentifier(this, 'saveButton', '<openmrs:message
                               code="error.identifier"/>');"/>
                <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
            </spring:bind>
        </td>
    ...
    <tr>
        ...
        <td class="patientIdentifierTypeColumn">
            <spring:bind path="identifierType">
                ...
                <select id="identifierTypeBox${varStatus.count == null ? 0 : varStatus.count}" name="${status.expression}"
                        onfocus="storeSelectedIdentifierType(this.options[this.selectedIndex].text)"
                        onclick="modifyTab(this, this.options[this.selectedIndex].text, 0);"
                        onChange="toggleLocationBoxAndIndentifierTypeWarning(this.options[this.selectedIndex].value,this.id);">
                    <option value=""></option>
                ...
                <div class="identifierTypeWarningWrapper" style="display:inline-block;">
                    <div style="display: none; background-color: #ffff88" class="identifierTypeWarningDiv"
                         id="identifierTypeWarning${varStatus.count == null ? 0 : varStatus.count}">
    ...
    <tr>
        <td>
            <c:if test="${identifierLocationUsed}">
                ...
                <span id="locationRequired${varStatus.count == null ? 0 : varStatus.count}" class="required"
                      style="${hideLocation || varStatus.count == null ? 'display: none;' : ''}">*</span>
            </c:if>
        </td>
        <td>
            <spring:bind path="location">
                <select id="locationBox${varStatus.count == null ? 0 : varStatus.count}" name="${status.expression}"
                        style="${hideLocation || varStatus.count == null ? 'display: none;' : ''}">
                    <option value=""></option>
                    <openmrs:forEachRecord name="location">
                        <option value="${record.locationId}"
                                <c:if test="${record.locationId == status.value}">selected</c:if>>
                            <c:out value="${record.name}"/>
                        </option>
                    </openmrs:forEachRecord>
                </select>
                <span id="locationNABox${varStatus.count == null ? 0 : varStatus.count}"
                      style="${hideLocation && varStatus.count != null ? '' : 'display: none;'}">
					<c:if test="${identifierLocationUsed}">
            ...
            <td>
                <spring:bind path="voided">
                    ...
                    <input type="checkbox" name="${status.expression}"
                           ...
                           onClick="toggleLayer('voidReasonIdentifierRow-<c:out
                                   value="${identifier}"/>'); if (voidedBoxClicked) voidedBoxClicked(this); "
                    />
                </spring:bind>
            </td>
        </tr>
        <tr id="voidReasonIdentifierRow-${identifier.patientIdentifierId} "
            <spring:bind path="voided">
            <c:if test="${status.value == false}">style="display: none"</c:if>
        </spring:bind> >
