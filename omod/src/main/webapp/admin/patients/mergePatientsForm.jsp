...
<c:if test="${empty msg}">
        ...
        <div class="box" style="max-height:160px; overflow:auto">
            <table class="box" cellspacing="2" cellpadding="2">
                <tr>
                    ...
                    <th><openmrs:message code="Patient.identifiers"/></th>
                    <th><openmrs:message code="PersonName.givenName"/></th>
                    <th><openmrs:message code="PersonName.middleName"/></th>
                    <th><openmrs:message code="PersonName.familyName"/></th>
                    <th><openmrs:message code="Person.age"/></th>
                    <th><openmrs:message code="Person.gender"/></th>
                    <th><openmrs:message code="Person.birthdate"/></th>
                </tr>
                <c:forEach items="${patientList}" var="patient" varStatus="status">
				...
				<c:otherwise>oddRow</c:otherwise>
                        ...
                        <td><input type="radio" id="${status.index}" name="preferred2"
                                   value="<c:out value="${patient.patientId}" />"
                                   onclick="display(this, true, ${patient.patientId})"
                                   <c:if test="${patient.voided==true}">disabled</c:if>/></td>
                        <td>${patient.patientId}</td>
                        <td><c:forEach items="${patient.identifiers}" var="identifier" varStatus="entries">
                            <c:if test="${entries.index==0}">${identifier}</c:if>
                        </c:forEach></td>
                        <c:forEach items="${patient.names}" var="name" varStatus="entries">
                            <c:if test="${entries.index==0}">
                                <td><c:out value="${name.givenName}"/></td>
                                <td><c:out value="${name.middleName}"/></td>
                                <td><c:out value="${name.familyName}"/></td>
                            </c:if></c:forEach>
                        <td>${patient.age}</td>
                        <td>
                            <c:choose>
    ...
    <form method="post">
        <table width="100%" id="patientTable" cellpadding="1" cellspacing="0">
            ...
            <c:if test="${patient2.patientId != null}">
                <tr>
                    <td valign="top">
                        <h4 id="p1">
                            <input type="radio" name="preferred1" id="<c:out value="${patient1.patientId}" />preferred"
                                   value="<c:out value="${patient1.patientId}" />"
                                   onclick="if (this.checked) changePrimary('left',${patient1.patientId})"
                                   <c:if test="${!patient1.voided}">checked</c:if>
                                       <c:if test="${patient1.voided}">disabled</c:if> />
                            <label for="<c:out value="${patient1.patientId}" />preferred"><openmrs:message
                                    code="Patient.merge.preferred"/></label>
                    ...
                    <td valign="top">
                        <h4 id="p2">
                            <input type="radio" name="preferred1" id="<c:out value="${patient2.patientId}" />preferred"
                                   value="<c:out value="${patient2.patientId}" />"
                                   onclick="if (this.checked) changePrimary('right', ${patient2.patientId})"
                                   <c:if test="${patient2.voided}">disabled</c:if>/>
                            <label for="<c:out value="${patient2.patientId}" />preferred"><openmrs:message
                                    code="Patient.merge.preferred"/></label>
            ...
            <tr id="tabsRow">
                ...
                <td class="mergePatientsTabsColumn">
                    <table cellpadding="0" cellspacing="0" width="100%">
                        <tr id="tabsRowRight">
                            <td class="mergePatientsTabSpaces" width="10px">&nbsp;</td>
                            <td id="lastTab" class="mergePatientsTabSpaces"></td>
                        </tr>
                    </table>
                </td>
            </tr>
                ...
                <td valign="top">
                    ...
                    <ol id="name1">
                        <c:forEach items="${patient1.names}" var="name">
                            <li><c:out value="${name.givenName}"/> <c:out value="${name.middleName}"/> <c:out
                                    value="${name.familyName}"/></li>
                        </c:forEach>
                    </ol>
                </td>
                <c:if test="${patient2.patientId == null}">
            ...
            <tr>
                <td>
                    <a href="patient.form?patientId=<c:out value="${patient1.patientId}" />" id="edit1"><openmrs:message
                            code="Patient.edit"/></a>
                </td>
                <c:if test="${patient2.patientId != null}">
                    <td>
                        <a href="patient.form?patientId=<c:out value="${patient2.patientId}" />" id="edit2"><openmrs:message
                                code="Patient.edit"/></a>
                    </td>
                </c:if>
            </tr>
        ...
        <c:if test="${patient2.patientId != null}">
            <br/>
            <input id="mergeButton" type="submit" name="action" value='<openmrs:message code="Patient.merge"/>'
                   onclick="return generateMergeList();">
