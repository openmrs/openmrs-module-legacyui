...
<c:if test="${patient.patientId != null}">
    <a href="${pageContext.request.contextPath}/patientDashboard.form?patientId=<c:out value="${patient.patientId}" />"><openmrs:message
            code="patientDashboard.viewDashboard"/></a>
    |
    <a href="${pageContext.request.contextPath}/admin/patients/mergePatients.form?patientId=<c:out value="${patient.patientId}" />"><openmrs:message
            code="Patient.mergeThis"/></a><br/><br/>
</c:if>
...
<openmrs:hasPrivilege privilege="Delete Patients">
    <c:if test="${patient.voided}">
        <div id="patientFormVoided" class="retiredMessage">
            ...
            <div>
                <c:if test="${patient.voidedBy.personName != null}"><openmrs:message code="general.byPerson"/> <c:out
                        value="${patient.voidedBy.personName}"/></c:if>
                <c:if test="${patient.dateVoided != null}"> <openmrs:message code="general.onDate"/> <openmrs:formatDate
                        date="${patient.dateVoided}" type="long"/> </c:if>
                <c:if test="${patient.voidReason != ''}"> - ${patient.voidReason} </c:if>
            </div>
            <div>
                <form action="" method="post"><input type="submit" name="action"
                                                     value="<openmrs:message code="Patient.unvoid"/>"/></form>
            </div>
        </div>
    </c:if>
...
<form method="post" onSubmit="removeBlankData()">
    ...
    <div id="pIds">
        <div class="tabBar" id="pIdTabBar">
            <c:forEach var="identifier" items="${patient.identifiers}" varStatus="varStatus">
                <a href="javascript:return false;" onClick="return selectTab(this, 'identifier');"
                   id="identifier${varStatus.index}" <c:if test="${identifier.voided}">class='voided'</c:if>><span><c:out
                        value="${identifier.identifierType.name}"/></span>&nbsp;</a>
            </c:forEach>
            <a href="javascript:return false;" onClick="return selectTab(this, 'identifier');" id="identifierTab"
               style="display: none"><span></span>&nbsp;</a>
            <input type="button" onClick="return addNew('identifier');" class="addNew" id="identifier"
                   value='<openmrs:message code="Patient.addNewIdentifier"/>'/>
        </div>
        <div class="tabBoxes" id="identifierDataBoxes">
            <c:forEach var="identifier" items="${patient.identifiers}" varStatus="varStatus">
                <spring:nestedPath path="patient.identifiers[${varStatus.index}]">
                    <div id="identifier${varStatus.index}Data" class="tabBox">
                        ...
                        <!-- <input type="button" onClick="return removeTab(this, 'identifier');" class="removeTab" value='<openmrs:message code="Patient.removeThisIdentifier"/>'/><br/> -->
                        <br/>
                    </div>
                </spring:nestedPath>
            </c:forEach>
            <div id="identifierData" class="tabBox">
                <spring:nestedPath path="emptyIdentifier">
                    <%@ include file="include/editPatientIdentifier.jsp" %>
                    <!-- <input type="button" onClick="return removeTab(this, 'identifier');" class="removeTab" value='<openmrs:message
                        code="Patient.removeThisIdentifier"/>'/><br/> --> <br/>
                </spring:nestedPath>
            </div>
        </div>
    </div>
    ...
    <div id="pNames">
        <div class="tabBar" id="pNameTabBar">
            <c:forEach var="name" items="${patient.names}" varStatus="varStatus">
                <a href="javascript:return false;" onClick="return selectTab(this, 'name');" id="name${varStatus.index}"
                   <c:if test="${name.voided}">class='voided'</c:if>><span><c:out
                        value="${name.givenName}"/></span>&nbsp;<span><c:out value="${name.familyName}"/></span></a>
            </c:forEach>
            <a href="javascript:return false;" onClick="return selectTab(this, 'name');" id="nameTab"
               style="display: none"><span></span>&nbsp;<span></span></a>
            <input type="button" onClick="return addNew('name');" class="addNew" id="name"
                   value='<openmrs:message code="Patient.addNewName"/>'/>
        </div>
        <div class="tabBoxes" id="nameDataBoxes">
            <c:forEach var="name" items="${patient.names}" varStatus="varStatus">
                <spring:nestedPath path="patient.names[${varStatus.index}]">
                    <div id="name${varStatus.index}Data" class="tabBox">
                        <openmrs:portlet url="nameLayout" id="namePortlet" size="full"
                                         parameters="layoutShowTable=true|layoutShowExtended=true|layoutHideVoidOption=${(name.personNameId == null)}"/>
                        <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message code="Patient.removeThisName"/>'/><br/> -->
                        <br/>
                    </div>
                </spring:nestedPath>
            </c:forEach>
            <div id="nameData" class="tabBox">
                <spring:nestedPath path="emptyName">
                    <openmrs:portlet url="nameLayout" id="namePortlet" size="full"
                                     parameters="layoutShowTable=true|layoutShowExtended=true|layoutHideVoidOption=true"/>
                    <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message
                        code="Patient.removeThisName"/>'/><br/> --> <br/>
                </spring:nestedPath>
            </div>
        </div>
    </div>
    ...
    <div id="pAddresses">
        <div class="tabBar" id="pAddressesTabBar">
            <c:forEach var="address" items="${patient.addresses}" varStatus="varStatus">
                <a href="javascript:return false;" onClick="return selectTab(this, 'address');"
                   id="address${varStatus.index}" <c:if test="${address.voided}">class='voided'</c:if>><span><c:out
                        value="${address.cityVillage}"/></span>&nbsp;</a>
            </c:forEach>
            <a href="javascript:return false;" onClick="return selectTab(this, 'address');" id="addressTab"
               style="display: none"><span></span>&nbsp;</a>
            <input type="button" onClick="return addNew('address');" class="addNew" id="address"
                   value='<openmrs:message code="Patient.addNewAddress"/>'/>
        </div>
        <div class="tabBoxes" id="addressDataBoxes">
                ...
                <spring:nestedPath path="patient.addresses[${varStatus.index}]">
                    <div id="address${varStatus.index}Data" class="tabBox">
                        <openmrs:portlet url="addressLayout" id="addressPortlet" size="full"
                                         parameters="layoutShowTable=true|layoutShowExtended=true|layoutHideVoidOption=${(address.personAddressId == null)}|isNew=${(address.personAddressId == null)}"/>
                            <%-- @ include file="include/editPersonAddress.jsp" --%>
                        <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message code="Patient.removeThisAddress"/>'/><br/> -->
                        <br/>
                    </div>
                </spring:nestedPath>
            </c:forEach>
            <div id="addressData" class="tabBox">
                <spring:nestedPath path="emptyAddress">
                    <openmrs:portlet url="addressLayout" id="addressPortlet" size="full"
                                     parameters="layoutShowTable=true|layoutShowExtended=true|layoutHideVoidOption=true|isNew=true"/>
                    <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message
                        code="Patient.removeThisAddress"/>'/><br/> --> <br/>
                </spring:nestedPath>
            </div>
        </div>
    </div>
    ...
    <c:if test="${patient.patientId != null}">
        <openmrs:hasPrivilege privilege="Purge Patients">
			...
			<input type="button" id="deletePatientButton" value="<openmrs:message code="Patient.delete"/>"
                   onClick="showDiv('deletePatientDiv'); hideDiv('deletePatientButton')"/>
			<div id="deletePatientDiv"
                 style="position: absolute; padding: 1em; bottom: 0px; left: 0px; z-index: 9; width: 350px; border: 1px black solid; background-color: #ffff88; display: none">
				...
				<div align="center">
					<input type="submit" name="action" value="<openmrs:message code="Patient.delete"/>"
                           onclick="return confirm('<openmrs:message code="Patient.delete.finalWarning"/>')"/>
					&nbsp; &nbsp; &nbsp;
					<input type="button" value="<openmrs:message code="general.cancel" />"
                           onClick="showDiv('deletePatientButton'); hideDiv('deletePatientDiv')"/>
				</div>
			</div>
		</span>
        </openmrs:hasPrivilege>
