...
<openmrs:require privilege="Manage Identifier Types" otherwise="/login.htm"
                 redirect="/admin/patients/patientIdentifierType.form"/>
            ...
            <tr>
                ...
                <td>
                    <spring:bind path="patientIdentifierType.formatDescription">
                        <input type="text" name="${status.expression}" value="${fn:replace(status.value, "\"", "&quot;")}"
                               size="50"/>
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
                </td>
                ...
                <td><spring:bind path="patientIdentifierType.locationBehavior">
                    <select name="${status.expression}">
                        ...
                        <c:forEach var="locationBehavior" items="${locationBehaviors}">
                            <option value="${locationBehavior}"
                                    <c:if test="${status.value == locationBehavior}">selected</c:if>>
                ...
                <td><spring:bind path="patientIdentifierType.uniquenessBehavior">
                    <select name="${status.expression}">
                        ...
                        <c:forEach var="uniquenessBehavior" items="${uniquenessBehaviors}">
                            <option value="${uniquenessBehavior}"
                                    <c:if test="${status.value == uniquenessBehavior}">selected</c:if>>
