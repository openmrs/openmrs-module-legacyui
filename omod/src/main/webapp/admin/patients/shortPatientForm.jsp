...
<form:form method="post" action="shortPatientForm.form" onsubmit="removeHiddenRows()" modelAttribute="patientModel">
    ...
    <table id="parentTable" cellspacing="0" cellpadding="7">
        <tr class="parentTableRow">
            ...
            <td class="inputCell">
                <table id="personNameTable" class="childTable" cellspacing="2">
                    <thead>
                    <openmrs:portlet url="nameLayout" id="namePortlet" size="columnHeaders"
                                     parameters="layoutShowTable=false|layoutShowExtended=false"/>
                    </thead>
                    <spring:nestedPath path="personName">
                        <openmrs:portlet url="nameLayout" id="namePortlet" size="inOneRow"
                                         parameters="layoutMode=edit|layoutShowTable=false|layoutShowExtended=false"/>
                    </spring:nestedPath>
                </table>
            </td>
        </tr>
            ...
            <td class="inputCell">
                <table id="identifiers" class="childTable" cellspacing="2">
                    <tr class="childTableRow">
                        <td class="idNumberHeaderColumn"><openmrs:message code="PatientIdentifier.identifier"/><span
                    ...
                    <c:forEach var="id" items="${patientModel.identifiers}" varStatus="varStatus">
                        ...
                        <spring:nestedPath path="identifiers[${varStatus.index}]">
                            <tr id="existingIdentifiersRow[${varStatus.index}]"
                                <c:if test="${id.voided}">style='display: none'</c:if>>
                                ...
                                <td class="idNumberDataColumn patientIdentifierTypeColumn" valign="top">
                                    <form:select path="identifierType"
                                                 onfocus="storeSelectedIdentifierType(this.options[this.selectedIndex].text)"
                                                 onchange="toggleLocationBoxAndIndentifierTypeWarning(this.options[this.selectedIndex].value,'initialLocationBox${varStatus.index}', '${varStatus.index}');">
                                        <form:option value=""></form:option>
                                        <form:options items="${identifierTypes}" itemValue="patientIdentifierTypeId"
                                                      itemLabel="name"/>
                                    </form:select>
                                </td>
                                <td class="idNumberDataColumn" valign="top">
                                    <c:set var="behavior" value="${id.identifierType.locationBehavior}"/>
                                    <div id="initialLocationBox${varStatus.index}"
                                         style="${(behavior == 'NOT_USED' || empty id.identifierType) ? 'display:none;' : ''}">
                                        ...
                                        <spring:bind path="location">
                                            <select id="locationId" name="${status.expression}">
                                                ...
                                                <c:forEach items="${locations}" var="loc">
                                                    <option value="${loc.locationId}" <c:if
                                                            test="${id.location.locationId == loc.locationId}"> selected="selected"</c:if>>
                                                        <c:out value="${loc.name}"/></option>
                                                </c:forEach>
                                            </select>
                                        </spring:bind>
                                    </div>
                                    <div id="initialLocationBox${varStatus.index}_NA"
                                         style="${behavior == 'NOT_USED' ? '' : 'display:none;'}">
                                        <c:if test="${identifierLocationUsed}">
                                ...
                                <td class="idNumberDataColumn" valign="middle" align="center">
                                    <spring:bind path="preferred">
                                        ...
                                        <input id="${status.expression}" type="radio" name="${status.expression}"
                                               value="true" onclick="updatePreferred(this)"
                                ...
                                <td class="idNumberDataColumn" valign="middle">
                                    <spring:bind path="voided">
                                        ...
                                        <input id="identifiers[${varStatus.index}].isVoided" type="checkbox"
                                               name="${status.expression}" value="${status.value}"
                                               <c:if test="${id.voided}">checked='checked'</c:if> style="display:none"/>
                                        <input type="button" name="closeButton"
                                               onClick="removeRow(this, 'identifiers[${varStatus.index}].isVoided','${varStatus.index}');"
                                               class="closeButton" value='<openmrs:message code="general.remove"/>'
                                               <c:if test="${(varStatus.first && varStatus.last)}">style="display: none;"</c:if> />
                                    </spring:bind>
                                </td>
                    ...
                    <tr id="newIdentifierRow" style="display: none">
                        ...
                        <td class="idNumberDataColumn" valign="top">
                            <select name="location" style="display: none;">
                                ...
                                <openmrs:forEachRecord name="location">
                                    <option value="${record.locationId}"<c:if
                                            test="${identifierLocationUsed && record == defaultLocation}"> selected="selected"</c:if>>
                                        <c:out value="${record.name}"/>
                                    </option>
                                </openmrs:forEachRecord>
                            </select>
                        ...
                        <td class="idNumberDataColumn" valign="middle" align="center">
                            ...
                            <input type="button" name="closeButton" class="closeButton"
                                   value='<openmrs:message code="general.remove"/>'/>
                        </td>
                        <td class="duplicateIdentifierTypeWarning">
                ...
                <input type="button" class="smallButton" onclick="addIdentifier(${fn:length(patientModel.identifiers)})"
                       value="<openmrs:message code="PatientIdentifier.add" />" hidefocus/>
            </td>
        </tr>
        <tr class="parentTableRow">
            ...
            <td class="inputCell">
                <table id="demographicsTable" class="childTable">
                    <tr class="childTableRow">
                        ...
                        <td class="demographicsHeaderColumn"><openmrs:message code="Person.birthdate"/> <i
                                style="font-weight: normal; font-size: 0.8em;">(<openmrs:message code="general.format"/>:
                            <openmrs:datePattern/>)</i></td>
                    </tr>
                    <tr class="childTableRow">
                        <td class="demographicsDataColumn" style="padding-right: 3em">
                            <spring:bind path="patient.gender">
                                <openmrs:forEachRecord name="gender">
                                    <input type="radio" name="${status.expression}" id="${record.key}" value="${record.key}"
                                           <c:if test="${record.key == status.value}">checked</c:if> />
                                    <label for="${record.key}"> <openmrs:message
                                            code="Person.gender.${record.value}"/> </label>
                                </openmrs:forEachRecord>
                        ...
                        <td class="demographicsDataColumn" style="padding-right: 3em">
							...
							<spring:bind path="patient.birthdateEstimated">
                                <label for="birthdateEstimatedInput"><openmrs:message
            ...
            <c:choose>
                <c:when test="${fn:contains(relationshipMap.key, 'a')}">
                    <tr class="parentTableRow">
                        ...
                        <td class="inputCell">
                            <openmrs_tag:personField formFieldName="${relationshipMap.key}" searchLabelCode="Person.find"
                                                     initialValue="${relationshipMap.value.personA.personId}" linkUrl=""
                                                     callback="" canAddNewPerson="true"/>
                        </td>
                    </tr>
                </c:when>
                <c:otherwise>
                    <tr class="parentTableRow">
                        ...
                        <td class="inputCell">
                            <openmrs_tag:personField formFieldName="${relationshipMap.key}" searchLabelCode="Person.find"
                                                     initialValue="${relationshipMap.value.personB.personId}" linkUrl=""
                                                     callback="" canAddNewPerson="true"/>
                        </td>
                    </tr>
                </c:otherwise>
        ...
        <c:forEach var="personAttribute" items="${patientModel.personAttributes}" varStatus="varStatus">
            ...
            <tr class="parentTableRow">
                <th class="headerCell"><openmrs:message
                        code="PersonAttributeType.${fn:replace(personAttribute.attributeType.name, ' ', '')}"
                        text="${personAttribute.attributeType.name}"/></th>
                <td class="inputCell">
                    <c:choose>
                ...
                <div id="deathInformation">
                    ...
                    <spring:bind path="patient.deathDate">
                        <input type="text" name="${status.expression}" size="10"
                               ...
                               id="deathDate"/>
                        <i style="font-weight: normal; font-size: 0.8em;">(<openmrs:message code="general.format"/>:
                    ...
                    <spring:bind path="patient.causeOfDeath">
                        <openmrs:fieldGen type="org.openmrs.Concept" formFieldName="${status.expression}"
                                          val="${status.value}"
                                          parameters="showAnswers=${conceptCauseOfDeath}|showOther=${conceptOther}|otherValue=${causeOfDeathOther}"/>
