...
<openmrs:require privilege="Manage Person Attribute Types" otherwise="/login.htm"
                 redirect="/admin/person/personAttributeType.form"/>
...
<form method="post">
    <fieldset>
        <table>
                    ...
                    <spring:bind path="personAttributeType.format">
                        ...
                        <select name="format">
                            ...
                            <c:forEach items="${formats}" var="format">
                                <option value="<c:out value="${format}"/>"
                                        <c:if test="${format == status.value}">selected</c:if>><c:out
                                        value="${format}"/></option>
                            </c:forEach>
                        ...
                        <c:if test="${isJavaUtilDate != ''}">
                            <br/>
                            <span class="error"><openmrs:message htmlEscape="false"
                                                                 code="PersonAttributeType.java.util.Date.warning"/></span>
                        </c:if>
                    </spring:bind>
                </td>
                        ...
                        <select name="editPrivilege">
                            ...
                            <c:forEach items="${privileges}" var="privilege">
                                <option value="<c:out value="${privilege.privilege}"/>"
                                        <c:if test="${privilege.privilege == status.value}">selected</c:if>><c:out
                                        value="${privilege.privilege}"/></option>
                            </c:forEach>
                        </select>
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
            ...
            <tr>
                <c:if test="${personAttributeType.personAttributeTypeId != null}">
                    ...
                    <td colspan="${fn:length(locales)}"><font color="#D0D0D0"><sub>${personAttributeType.uuid}</sub></font>
                    </td>
                </c:if>
            </tr>
        </table>
        ...
        <input type="submit" value="<openmrs:message code="PersonAttributeType.save"/>" name="save" <c:if
                test="${PersonAttributeTypesLocked == 'true'}"> disabled</c:if>>
    </fieldset>
</form>
...
<c:if test="${not personAttributeType.retired && not empty personAttributeType.personAttributeTypeId}">
    <form method="post">
        <fieldset>
            ...
            <br/>
            <input type="submit" value='<openmrs:message code="PersonAttributeType.retirePersonAttributeType"/>'
                   name="retire" <c:if test="${PersonAttributeTypesLocked == 'true'}"> disabled</c:if> />
        </fieldset>
    </form>
</c:if>
    ...
    <openmrs:hasPrivilege privilege="Purge Person Attribute Types">
        <form id="purge" method="post" onsubmit="return confirmPurge()">
            <fieldset>
                ...
                <input type="submit" value='<openmrs:message code="PersonAttributeType.purgePersonAttributeType"/>'
                       name="purge" <c:if test="${PersonAttributeTypesLocked == 'true'}"> disabled</c:if> />
            </fieldset>
        </form>
    </openmrs:hasPrivilege>
</c:if>
