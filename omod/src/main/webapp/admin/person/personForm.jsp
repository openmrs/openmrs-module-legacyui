...
<openmrs:hasPrivilege privilege="Delete Person">
    <c:if test="${person.personVoided}">
        <div id="personFormVoided" class="retiredMessage">
            ...
            <div>
                <c:if test="${person.personVoidedBy.personName != null}"><openmrs:message code="general.byPerson"/> <c:out
                        value="${person.personVoidedBy.personName}"/></c:if>
                <c:if test="${person.personDateVoided != null}"> <openmrs:message code="general.onDate"/> <openmrs:formatDate
                        date="${person.personDateVoided}" type="long"/> </c:if>
                <c:if test="${person.personVoidReason != ''}"> - ${person.personVoidReason} </c:if>
            </div>
            <div>
                <form action="" method="post"><input type="submit" name="action"
                                                     value="<openmrs:message code="Person.unvoid"/>"/></form>
            </div>
        </div>
    </c:if>
...
<form method="post" onSubmit="removeBlankData()">
    ...
    <div id="pNames">
        <div class="tabBar" id="pNameTabBar">
            ...
            <input type="button" onClick="return addNew('name');" class="addNew" id="name"
                   value='<openmrs:message code="Person.addNewName"/>'/>
        </div>
            ...
            <c:forEach var="name" items="${person.names}" varStatus="varStatus">
                <spring:nestedPath path="person.names[${varStatus.index}]">
                    <div id="name${varStatus.index}Data" class="tabBox">
                        ...
                        <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message code="Person.removeThisName"/>'/><br/> -->
                        <br/>
                    </div>
                </spring:nestedPath>
            </c:forEach>
            <div id="nameData" class="tabBox">
                <spring:nestedPath path="emptyName">
                    ...
                    <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message
                        code="Person.removeThisName"/>'/><br/> --> <br/>
                </spring:nestedPath>
            </div>
        </div>
    </div>
        ...
        <div class="tabBar" id="pAddressesTabBar">
            <c:forEach var="address" items="${person.addresses}" varStatus="varStatus">
                <a href="javascript:return false;" onClick="return selectTab(this, 'address');"
                   id="address${varStatus.index}"
                   <c:if test="${address.voided}">class='voided'</c:if>><span>${address.cityVillage}</span>&nbsp;</a>
            </c:forEach>
            ...
            <input type="button" onClick="return addNew('address');" class="addNew" id="address"
                   value='<openmrs:message code="Person.addNewAddress"/>'/>
        </div>
        <div class="tabBoxes" id="addressDataBoxes">
            <c:forEach var="address" items="${person.addresses}" varStatus="varStatus">
                <spring:nestedPath path="person.addresses[${varStatus.index}]">
                    <div id="address${varStatus.index}Data" class="tabBox">
                        ...
                        <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message code="Person.removeThisAddress"/>'/><br/> -->
                        <br/>
                    </div>
                </spring:nestedPath>
            </c:forEach>
            <div id="addressData" class="tabBox">
                <spring:nestedPath path="emptyAddress">
                    ...
                    <!-- <input type="button" onClick="return removeTab(this, 'name');" class="removeTab" value='<openmrs:message
                        code="Person.removeThisAddress"/>'/><br/> --> <br/>
                </spring:nestedPath>
            </div>
        </div>
    </div>
    ...
    <c:if test="${person.personId != null}">
        <openmrs:hasPrivilege privilege="Purge Person">
			...
			<input type="button" id="deletePersonButton" value="<openmrs:message code="Person.delete"/>"
                   onClick="showDiv('deletePersonDiv'); hideDiv('deletePersonButton')"/>
			<div id="deletePersonDiv"
				...
				<br/><br/>
				<div align="center">
					<input type="submit" name="action" value="<openmrs:message code="Person.delete"/>"
                           onclick="return confirm('<openmrs:message code="Person.delete.finalWarning"/>')"/>
					&nbsp; &nbsp; &nbsp;
					<input type="button" value="<openmrs:message code="general.cancel" />"
                           onClick="showDiv('deletePersonButton'); hideDiv('deletePersonDiv')"/>
				</div>
			</div>
		</span>
        </openmrs:hasPrivilege>
