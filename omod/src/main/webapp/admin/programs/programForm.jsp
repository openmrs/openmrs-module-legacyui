...
<script type="text/javascript">
    ...
    function refreshWorkflowsDisplay() {
        ...
        dwr.util.addRows(tableId, values, [
            ...
            function (id) {
                return '<a href="javascript:removeWorkflow(' + id + ')">[x]</a>';
            },
        ], {escapeHtml: false});
    }
</script>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <spring:bind path="program.concept">
                    <div dojoType="ConceptSearch" widgetId="cSearch" conceptId="${status.value}" showVerboseListing="false"
                         conceptClasses="Program"></div>
                    <div dojoType="OpenmrsPopup" widgetId="conceptSelection" hiddenInputName="${status.expression}"
                         searchWidget="cSearch" searchTitle='<openmrs:message code="Concept.find" />'></div>
            ...
            <td valign="top">
                <spring:bind path="program.retired">
                    <select name="${status.expression}">
                        <option value="false" <c:if test="${status.value == false}">selected</c:if>><openmrs:message
                                code="general.no"/></option>
                        <option value="true" <c:if test="${status.value == true}">selected</c:if>><openmrs:message
                                code="general.yes"/></option>
                    </select>
                    <c:if test="${status.errorMessage != ''}">
        ...
        <tr id="workflowSetRow">
            ...
            <td valign="top">
                ...
                <div dojoType="OpenmrsPopup" widgetId="conceptSelection2" hiddenInputName="notUsed" searchWidget="wfSearch"
                     searchTitle='<openmrs:message code="Concept.find" />'
                     changeButtonValue='<openmrs:message code="general.add"/>'></div>
        ...
        <tr>
            ...
            <td>
                <spring:bind path="program.outcomesConcept">
                    <div dojoType="ConceptSearch" widgetId="oSearch" conceptId="${status.value}" showVerboseListing="false"
                         conceptClasses="Program"></div>
                    <div dojoType="OpenmrsPopup" widgetId="outcomeConceptSelection" hiddenInputName="${status.expression}"
                         searchWidget="oSearch" searchTitle='<openmrs:message code="Concept.find" />'></div>
