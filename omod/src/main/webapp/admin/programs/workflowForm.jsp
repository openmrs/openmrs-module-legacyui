    ...
    function refreshStateTable() {
        ...
        if (displayedStates.length != 0) {
            dwr.util.addRows('stateTable', displayedStates, [
                function (st) {
                    return getIdToNameMap(st[0]);
                },
                function (st) {
                    return '<input type="checkbox" id="initial_' + st[0] + '" ' + (st[1] ? 'checked' : '') + '/>';
                },
                function (st) {
                    return '<input type="checkbox" id="terminal_' + st[0] + '" ' + (st[2] ? 'checked' : '') + '/>';
                },
                function (st) {
                    return getButton(st[0]);
                },
                function (st) {
                    return getDeleteButton(st[0]);
                }
            ], {escapeHtml: false});
        } else {
...
<form method="post" id="theForm">
    <table>
        ...
        <tbody id="stateTable">
        <tr>
            <td colspan="3"><openmrs:message code="general.none"/></td>
        </tr>
        </tbody>
        <tbody>
        <tr>
            <td colspan="3" align="center">
                <div dojoType="ConceptSearch" widgetId="cSearch" conceptId="" showVerboseListing="false"
                     conceptClasses="State"></div>
                <div dojoType="OpenmrsPopup" widgetId="conceptSelection" hiddenInputName="conceptId" searchWidget="cSearch"
