        ...
        <table cellpadding="3" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <td>
                            <spring:bind path="provider.person">
                                <openmrs:fieldGen type="org.openmrs.Person" formFieldName="${status.expression}"
            ...
            <c:if test="${ not empty providerAttributeTypes }">
                <c:forEach var="attrType" items="${ providerAttributeTypes }">
                    <openmrs_tag:attributesForType attributeType="${ attrType }" customizable="${ provider }"
                                                   formFieldNamePrefix="attribute.${ attrType.providerAttributeTypeId }"/>
                </c:forEach>
            </c:if>
            <tr>
                <c:if test="${provider.providerId != null}">
        ...
        <form method="post">
            <table cellpadding="3" cellspacing="0">
                <tr>
                    ...
                    <td>
                        <a href="#View User" onclick="return gotoUser(null, '${provider.creator.userId}')"><c:out
                                value="${provider.creator.personName}"/></a> -
                    ...
                    <tr id="retiredReason">
                        ...
                        <td>
                            ...
                            <spring:bind path="provider.retireReason">
                                <input type="text" id="retire" value="${status.value}" name="${status.expression}"
                                       size="40"/>
                                <c:if test="${status.errorMessage != ''}"><span
                                        class="error">${status.errorMessage}</span></c:if>
                            </spring:bind>
                ...
                <c:if test="${provider.retiredBy != null}">
                    <tr id="retiredBy">
                        ...
                        <td>
                            <a href="#View User" onclick="return gotoUser(null, '${provider.retiredBy.userId}')"><c:out
                                    value="${provider.retiredBy.personName}"/></a> -
