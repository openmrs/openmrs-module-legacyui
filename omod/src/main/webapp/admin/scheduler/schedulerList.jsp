...
<style>
    #schedulerTable tr.top, #schedulerTable tr.bottom {
    ...
    #schedulerTable th.left, #schedulerTable td.left {
        text-align: left;
    }

    #schedulerTable td.button {
        border: 0;
    }

    #schedulerTable tr.even {
                ...
                <table cellpadding="0" cellspacing="0" width="100%" id="schedulerTable">
                    ...
                    <c:forEach var="task" items="${taskList}" varStatus="varStatus">
                        ...
                        <tr class="${styleClass}">
                                ...
                                <c:choose>
                                    <c:when test="${task.started}">
                                        ...
                                        <c:if test="${task.startTime!=null}">
                                            <i><openmrs:message htmlEscape="false" code="Scheduler.list.runsAgainIn"
                                                                arguments="${task.secondsUntilNextExecutionTime}"/></i>
                                        </c:if>
                                    </c:when>
                                    <c:otherwise>
                                        <font color="red"><strong><openmrs:message
                                                code="Scheduler.list.stopped"/></strong></font>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                            <td class="left">
                            ...
                            <td class="left"><openmrs:message htmlEscape="false" code="Scheduler.list.runsEvery"
                                                              arguments="${intervals[task]}"/>
                                <c:if test="${task.startTime!=null}">
                                    <br/><openmrs:message htmlEscape="false" code="Scheduler.list.from"
                                                          arguments="${taskStartTime}"/> ,
                                    <br/><openmrs:message htmlEscape="false" code="Scheduler.list.startingOn"
                                                          arguments="${taskStartDate}"/>
                                </c:if>
                            </td>
                            <td class="left">
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${task.startOnStartup}"><strong><openmrs:message
                                            code="general.yes"/></strong> (<openmrs:message code="Scheduler.list.automatic"/>)</c:when>
                                    <c:otherwise><strong><openmrs:message code="general.no"/></strong> (<openmrs:message
                                            code="Scheduler.list.manual"/>)</c:otherwise>
                                </c:choose>
                            </td>
                    ...
                    <tr class="bottom">
                        <td colspan="6">
                            ...
                            <input type="submit" value="<openmrs:message code="Scheduler.taskList.delete"/>"
                                   onclick="return confirm('<openmrs:message code="Scheduler.taskList.delete.warning"/>')"
                                   name="action">
                        </td>
                    </tr>
                </table>
