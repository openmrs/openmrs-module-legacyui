...
<form method="post">
    <table>
        ...
        <tr>
            ...
            <td valign="top">
                <input type="hidden" name="userIds" id="userIds" size="40"
                       value='<c:forEach items="${alert.recipients}" var="recipient">${recipient.recipient.userId} </c:forEach>'/>
                <table cellpadding="0" cellspacing="0">
                    <tr>
                        <td valign="top">
                            <select class="mediumWidth" size="6" id="userNames" multiple
                                    onkeyup="listKeyPress('userNames', 'userIds', ' ', event);">
                                <c:forEach items="${alert.recipients}" var="recipient">
                                    <option value="${recipient.recipient.userId}"><c:out
                                            value="${recipient.recipient.personName}"/></option>
                                </c:forEach>
                            </select>
                        </td>
                        <td valign="top" class="buttons">
                            &nbsp;<span dojoType="UserSearch" widgetId="uSearch"></span><span dojoType="OpenmrsPopup"
                                                                                              searchWidget="uSearch"
                                                                                              searchTitle='<openmrs:message code="User.find"/>'
                                                                                              changeButtonValue='<openmrs:message code="general.add"/>'></span>
                            <br/>
                            &nbsp; <input type="button" value="<openmrs:message code="general.remove"/>" class="smallButton"
                                          onClick="removeItem('userNames', 'userIds', ' ');"/> <br/>
                            &nbsp; <input type="button" value="<openmrs:message code="User.goTo"/>" class="smallButton"
                                          onClick="gotoUser('userNames');"/><br/>
                        </td>
                    </tr>
                </table>
            </td>
            ...
            <td valign="top">
                <select id="roleStr" class="mediumWidth">
                    ...
                    <c:forEach items="${allRoles}" var="role">
                        <option value="${role.role}" <c:if test="${role == status.value}">selected</c:if>><c:out
                                value="${role.role}"/></option>
                    </c:forEach>
                </select>
                &nbsp;<input type="button" class="smallButton" value="<openmrs:message code="Alert.addRole"/>"
                             onClick="addRole();"/>
        ...
        <tr>
            ...
            <td>
                <table cellpadding="0" cellspacing="0">
                    <tr>
                        <td>
                            <select class="mediumWidth" size="3" multiple id="roles"
                                    onkeydown="listKeyPress('roles', 'newRoles', ',', event);">
                            </select>
                        </td>
                        <td valign="top" class="buttons">
                            &nbsp; <input type="button" value="<openmrs:message code="general.remove"/>" class="smallButton"
                                          onClick="removeItem('roles', 'newRoles', ',');"/> <br/>
                        </td>
                    </tr>
                </table>
            </td>
            ...
            <td valign="top">
                <spring:bind path="alert.satisfiedByAny">
                    ...
                    <input type="checkbox" name="${status.expression}"
                           value="on"
                           <c:if test="${alert.satisfiedByAny}">checked</c:if> />
                </spring:bind>
        ...
        <c:if test="${!(encounter.changedBy == null)}">
            <tr>
                ...
                <td>
                    <a href="#View User" onclick="return gotoUser(null, '${alert.changedBy.userId}')"><c:out
                            value="${alert.changedBy.personName}"/></a> -
