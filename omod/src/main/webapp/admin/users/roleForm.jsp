    ...
    <table>
        <tr>
            ...
            <td>
                <spring:bind path="role.role">
                    <c:if test="${param.roleName == null}"><input type="text" id="role" name="${status.expression}"
                                                                  value="${status.value}" onChange="updateRoleName()"></c:if>
        ...
        <tr>
            ...
            <td valign="top">
                <spring:bind path="role.description">
                    <textarea name="description" rows="3" cols="50" onKeyUp="formChanged = true;"
                ...
                <td>
                    <table cellpadding="3">
                        <tr>
                            ...
                            <c:if test="${varStatus.index % 2 == 0}"></tr>
                        <tr></c:if>
                            <td><a href="role.form?roleName=<c:out value="${role.role}"/>" onclick="return leaveForm()"
                                   title="<c:out value="${role.description}"/>"><c:out value="${role.role}"/></a></td>
                            </c:forEach>
                        </tr>
                    </table>
                </td>
        ...
        <tr>
            ...
            <td>
                ...
                <br/>
                <c:if test="${role.role == superuser}"><openmrs:message
                        code="Role.superuser.hasAllRolesAndPrivileges"/></c:if>
                <c:if test="${role.role != superuser}">
                    <openmrs:listPicker name="inheritedRoles" allItems="${allRoles}" currentItems="${role.inheritedRoles}"
                                        descendantItems="${role.allChildRoles}"/>
                </c:if>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <c:if test="${role.role == superuser}"><openmrs:message
                        code="Role.superuser.hasAllRolesAndPrivileges"/></c:if>
                <c:if test="${role.role != superuser}">
                    <div class="listItem listItemSelectAll"><input type="checkbox"
                                                                   id="toggleSelectionCheckbox"><spring:message
                            code="general.selectOrUnselectAll"/></div>
                    <openmrs:listPicker name="privileges" allItems="${privileges}" currentItems="${role.privileges}"
                                        inheritedItems="${inheritedPrivileges}"/>
                </c:if>
            </td>
        </tr>
        <tr>
