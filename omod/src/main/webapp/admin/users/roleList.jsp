            ...
            <tr class='${rowStatus.index % 2 == 0 ? "evenRow" : "oddRow" }'>
                ...
                <c:choose>
                    <c:when test="${map.key.role == superuser}">
                        <td colspan="2" style="white-space: nowrap"><openmrs:message
                                code="Role.superuser.hasAllRolesAndPrivileges"/></td>
                    </c:when>
                    <c:otherwise>
                        <td style="white-space: nowrap"
                            <c:if test="${fn:length(map.key.inheritedRoles)>2}">title="<c:out value="${map.key.inheritedRoles}"/>"
                        </c:if>>
                        ...
                        <td style="white-space: nowrap"
                            <c:if test="${fn:length(map.key.privileges)>2}">title="<c:out value="${map.key.privileges}"/>"
                        </c:if>>
