        ...
        <table>
            <tr valign="top">
                ...
                <td style="padding-left: 5em">
                    ...
                    <form method="get" action="user.form">
                        <openmrs:message code="User.whichPerson"/> <openmrs_tag:personField formFieldName="person_id"
                                                                                            formFieldId="existingPersonId"
                                                                                            callback="personSelectedCallback"/>
                        <br/>
                        <input id="useExistingButton" disabled="true" type="submit"
                               value="<openmrs:message code="general.next"/>"/>
                    </form>
                </td>
            </tr>
        </table>
    ...
    <c:otherwise>
        ...
        <form id="thisUserForm" method="post" action="user.form" autocomplete="off">
                ...
                <table>
                    ...
                    <c:if test="${modifyPasswords == true}">
                        <tr>
                            ...
                            <td><input type="password" name="userFormPassword"
                                       value="<c:if test="${isNewUser == false}">XXXXXXXXXXXXXXX</c:if>" autocomplete="off"/>
                                ...
                                <openmrs:globalProperty key="security.passwordRequiresNonDigit"
                                                        var="passwordRequiresNonDigit"/>
                                <openmrs:globalProperty key="security.passwordRequiresUpperAndLowerCase"
                                                        var="passwordRequiresUpperAndLowerCase"/>

                                <i><openmrs:message code="general.passwordLength" arguments="${passwordMinimumLength}"/>

                                    <% boolean prevCondition = false; %>

                                    <c:if test="${passwordRequiresUpperAndLowerCase == true || passwordRequiresDigit == true || passwordRequiresNonDigit == true}">
                                        <openmrs:message code="general.shouldHave"/></c:if>

                                    <c:if test="${passwordRequiresUpperAndLowerCase == true}"> <openmrs:message
                                            code="changePassword.hint.password.bothCasesRequired"/><% prevCondition = true; %>
                                    </c:if>

                                    <c:if test="${passwordRequiresDigit == true}"><% if (prevCondition == true)
                                        out.print(","); %> <openmrs:message
                                            code="changePassword.hint.password.digitRequired"/><% prevCondition = true; %>
                                    </c:if>

                                    <c:if test="${passwordRequiresNonDigit == true}"><% if (prevCondition == true)
                                        out.print(","); %> <openmrs:message
                                            code="changePassword.hint.password.nonDigitRequired"/></c:if>
                    ...
                    <tr>
                        <td colspan="2"><a href="#Show Advanced"
                                           onclick="return toggleLayer('advancedOptions', this, '<openmrs:message
                                                   code="User.showAdvancedOptions"/>', '<openmrs:message
                                                   code="User.hideAdvancedOptions"/>')"><openmrs:message
                                code="User.showAdvancedOptions"/></a></td>
                    </tr>
                    <tbody id="advancedOptions" style="display: none">
                    <c:if test="${modifyPasswords == true}">
                        <tr>
                            <td><openmrs:message code="User.secretQuestion"/></td>
                            <td><input type="text" name="secretQuestion" size="50" value="${secretQuestion}"/>
                                <i><openmrs:message code="general.optional"/></i></td>
                        </tr>
                        <tr>
                            <td><openmrs:message code="User.secretAnswer"/></td>
                            <td><input type="password" autocomplete="off" name="secretAnswer" size="50" value=""/>
                                <i><openmrs:message code="general.optional"/></i></td>
                        </tr>
                    </c:if>
                    <tr>
            ...
            <c:if test="${isNewUser == false}">
                <fieldset>
                    <legend><openmrs:message code="User.creationInfo"/></legend>
                    <table>
                        <tr>
            ...
            <c:if test="${user.retired == true}">
                <br/>
                <fieldset>
                    <legend><openmrs:message code="User.retiredInfo"/></legend>
                    <table>
                        <tr>
            ...
            <c:if test="${user.userId != null}">
                <c:if test="${!user.retired}">
                    <openmrs:hasPrivilege privilege="Become User (Actually you need to be a superuser)">
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <input type="submit" name="action" value="<openmrs:message code="User.assumeIdentity" />"
                               onClick="return confirm('<openmrs:message code="User.assumeIdentity.confirm"/>');"/>
                    </openmrs:hasPrivilege>
                </c:if>
                <openmrs:hasPrivilege privilege="Delete User">
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="submit" name="action" value="<openmrs:message code="User.delete" />"
                           onClick="return confirm('<openmrs:message code="User.delete.confirm"/>');"/>
                </openmrs:hasPrivilege>
            </c:if>
        </form>
