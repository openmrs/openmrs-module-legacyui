...
<form method="get">
    ...
    <table>
                ...
                <select name="role">
                    ...
                    <openmrs:forEachRecord name="role">
                        <c:if test="${record.role != 'Anonymous' && record.role != 'Authenticated'}">
                            <option <c:if test="${param.role == record.role}">selected</c:if>><c:out
                                    value="${record.role}"/></option>
                        </c:if>
        ...
        <tr>
            ...
            <td>
                <input type="checkbox" name="includeDisabled"
                       <c:if test="${param.includeDisabled == 'on'}">checked=checked</c:if>/>
            </td>
        </tr>
        <tr>
            <td></td>
        ...
        <table class="openmrsSearchTable" style="width: 100%;" cellpadding="2" cellspacing="0">
            ...
            <tr style="" dojoattachpoint="headerRow">
                ...
                <openmrs:forEachDisplayAttributeType personType="user" displayType="listing" var="attrType">
                    <th><openmrs:message code="PersonAttributeType.${fn:replace(attrType.name, ' ', '')}"
                            ...
                            <c:choose>
                                ...
                                <c:when test="${r != role && role != null}">
									<span class='bold_text'>
										<c:forEach var="inheritedRole" items="${userInheritanceLineMap[user]}"
