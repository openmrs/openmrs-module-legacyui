...
<script type="text/javascript">
    ...
    function addEncounter() {
        ...
        new AutoComplete('visitEncounters[' + index + ']-display', new CreateCallback({
            maxresults: 100,
            patientId:<c:out value="${param.patientId}" />
        }).encounterCallback(), {
    ...
    <form:form action="unvoidVisit.htm" method="post" modelAttribute="visit">
        ...
        <div class="voidedMessage">
            <div>
                ...
                <c:if test="${visit.voidedBy.personName != null}"> <openmrs:message code="general.byPerson"/> <openmrs:format
                        user="${visit.voidedBy}"/></c:if>
                <c:if test="${visit.dateVoided != null}"> <openmrs:message code="general.onDate"/> <openmrs:formatDate
                        date="${visit.dateVoided}" type="long"/></c:if>
...
<form:form method="post" action="visit.form" modelAttribute="visit">
    ...
    <div class="box">
            ...
            <div style="float: right">
                <c:if test="${visit.stopDatetime == null}">
                    <openmrs:hasPrivilege privilege="Edit Visits">
                        <input type="button" value="<openmrs:message code="Visit.end"/>"
                               onclick="javascript:$j('#endvisit-dialogue').dialog('open');	"/>
                ...
                <openmrs:hasPrivilege privilege="Delete Visits">
                    <c:if test="${visit.voided == false}">
                        ...
                        <input type="button" value='<openmrs:message code="general.void"/>'
                               onclick="javascript:$j('#delete-dialog').dialog('open')"/>
                    </c:if>
                </openmrs:hasPrivilege>
                ...
                <openmrs:hasPrivilege privilege="Purge Visits">
                    ...
                    <input type="button" value='<openmrs:message code="general.purge"/>'
                           onclick="javascript:$j('#purge-dialog').dialog('open')"
                            <c:if test="${!canPurgeVisit}"> disabled="disabled" title="<openmrs:message
                                    code="Visit.cannotPurgeVisitWithEncounters"/>"</c:if> />
                </openmrs:hasPrivilege>
            </div>
        </c:if>
        ...
        <table class="left-aligned-th" cellpadding="3" cellspacing="3">
            <tr>
                <th>
                    <openmrs:message code="general.patient"/><c:if test="${visit.visitId == null}"><span
                        ...
                        <select name="${status.expression}">
                            ...
                            <optgroup label="<openmrs:message code="Visit.type.retired"/>">
                                ...
                                <option value="${visitType.visitTypeId}"
                                        <c:if test="${visitType.visitTypeId == status.value}">selected="selected"</c:if>>
            ...
            <tr>
                ...
                <td>
                    <spring:bind path="indication">
                        <openmrs_tag:conceptField formFieldName="${status.expression}" formFieldId="conceptId"
                                                  initialValue="${status.value}"/>
                        <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                    </spring:bind>
                </td>
            ...
            <c:forEach var="attrType" items="${ attributeTypes }">
                <openmrs_tag:attributesForType attributeType="${ attrType }" customizable="${ visit }"
            ...
            <c:if test="${visit.visitId != null}">
                <c:if test="${visit.creator != null}">
                    <tr>
                        ...
                        <td>
                            <c:out value="${visit.creator.personName}"/> - <openmrs:formatDate date="${visit.dateCreated}"
                                                                                               type="long"/>
                        </td>
                    </tr>
                </c:if>
                <c:if test="${visit.changedBy != null}">
                    <tr>
                        ...
                        <td>
                            <c:out value="${visit.changedBy.personName}"/> - <openmrs:formatDate date="${visit.dateChanged}"
                                                                                                 type="long"/>
                        </td>
                    </tr>
                </c:if>
    ...
    <div class="box">
        ...
        <input type="button" value='<openmrs:message code="Visit.addEncounter"/>' class="smallButton"
    ...
    <c:if test="${not empty param.patientId}">
        <c:set var="cancelUrl"
               value="${pageContext.request.contextPath}/patientDashboard.form?patientId=${param.patientId}"/>
    </c:if>
    <input type="button" style="margin-left: 15px" value='<openmrs:message code="general.cancel" />'
           onclick='javascript:window.location="${cancelUrl}"'/>
    ...
    <div id="purge-dialog" title="<openmrs:message code="Visit.confirm.purge"/>">
        <form:form action="purgeVisit.htm" method="post" modelAttribute="visit">
            ...
            <table cellpadding="3" cellspacing="30" align="center">
                <tr>
                    <td>
                        <input type="submit" value='<openmrs:message code="general.yes"/>'/> &nbsp; <input type="button"
                                                                                                           value="<openmrs:message code="general.no"/>"
        ...
        <table cellpadding="3" cellspacing="3" align="center">
            <tr>
                <td>
                    ...
                    <input type="text" id="enddate_visit" size="20" name="stopDate"
                           value="<openmrs:formatDate date="${now}" format="dd/MM/yyyy HH:mm"/>"
                           onClick="showDateTimePicker(this)" readonly="readonly"/></br>&nbsp;&nbsp;
                </td>
            </tr>
            <tr height="20"></tr>
            <tr>
