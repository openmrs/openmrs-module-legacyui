...
<openmrs:message var="pageTitle" code="Concept.view.titlebar" scope="page" htmlEscape="true"
                 arguments="${command.concept.name}"/>
    ...
    <form class="inlineForm" id="jumpForm" action="" method="post">
        ...
        <c:if test="${conceptsLocked != 'true'}">
            <openmrs:hasPrivilege
                    privilege="Manage Concepts"><a href="concept.form?conceptId=${command.concept.conceptId}" id="editConcept" accesskey="e" valign="middle"></openmrs:hasPrivilege><openmrs:message
                code="general.edit"/><openmrs:hasPrivilege privilege="Manage Concepts"></a></openmrs:hasPrivilege> |
        </c:if>
        <a href="conceptStats.form?conceptId=${command.concept.conceptId}" accesskey="s" id="conceptStats"
...
<c:if test="${conceptsLocked != 'true'}">
    | <openmrs:hasPrivilege
        privilege="Manage Concepts"><a href="concept.form" id="newConcept" valign="middle"></openmrs:hasPrivilege><openmrs:message
        code="general.new"/><openmrs:hasPrivilege privilege="Manage Concepts"></a></openmrs:hasPrivilege>
</c:if>
...
<c:if test="${command.concept.conceptId != null}">
    ...
    <c:if test="${command.concept.retired}">
        <div class="retiredMessage">
            ...
            <div><c:if test="${command.concept.retiredBy.personName != null}"> <openmrs:message code="general.byPerson"/>
                <c:out value="${command.concept.retiredBy.personName}"/> </c:if> <c:if
                    test="${command.concept.dateRetired != null}"> <openmrs:message code="general.onDate"/>
                <openmrs:formatDate date="${command.concept.dateRetired}" type="long"/> </c:if> <c:if
                    test="${command.concept.retireReason!=''}"> - <c:out value="${command.concept.retireReason}"/>
            </c:if></div>
        </div>
    </c:if>
    ...
    <table id="conceptTable" cellpadding="2" cellspacing="0">
        ...
        <tr class="localeSpecific">
            <th valign="top" title="<openmrs:message code="Concept.synonyms.help"/>"><openmrs:message
                    code="Concept.synonyms"/></th>
            <c:forEach items="${command.locales}" var="loc">
                <td class="${loc}">
        ...
        <tr class="localeSpecific">
            <th valign="top" title="<openmrs:message code="Concept.indexTerms.help"/>"><openmrs:message
                    code="Concept.indexTerms"/></th>
            <c:forEach items="${command.locales}" var="loc">
                <td class="${loc}">
        ...
        <c:if test="${command.concept.set}">
            <tr id="setOptions">
                ...
                <td valign="top">
                    <c:if test="${fn:length(command.concept.setMembers) == 0}"><openmrs:message
                            code="Concept.conceptSets.empty"/></c:if>
                    <c:forEach items="${command.concept.setMembers}" var="setMember">
                        <a href="concept.htm?conceptId=${setMember.conceptId}"
                           <c:if test="${setMember.retired}">class="retired"</c:if>><openmrs:format concept="${setMember}"/>
                            (${setMember.conceptId})</a><br/>
                    </c:forEach>
                </td>
            </tr>
        </c:if>
                ...
                <c:forEach items="${command.locales}" var="loc">
                    <td class="${loc}">
                        <c:forEach items="${command.conceptAnswersByLocale[loc]}" var="answer">
                            <a href="concept.htm?conceptId=${fn:substring(answer.key, 0, fn:indexOf(answer.key, '^'))}"><c:out
                                    value="${answer.value}"/>
                                (${fn:substring(answer.key, 0, fn:indexOf(answer.key, '^'))})</a><br/>
                        </c:forEach>
                    </td>
                </c:forEach>
            </tr>
        ...
        <tr id="conceptMapRow">
            ...
            <td>
                <table cellpadding="5" cellspacing="3" align="left" class="lightBorderBox">
                    <tr id="conceptMappingsHeadersRow"
                        <c:if test="${fn:length(command.conceptMappings) == 0}">style="display:none"</c:if>>
        ...
        <c:forEach var="attribute" items="${command.concept.activeAttributes}">
            <tr id="conceptAttributeRow">
                <th valign="top" style="padding-top: 8px"
                    title="<openmrs:message code="attribute.attributeType.description"/>">
                    <c:out value="${attribute.attributeType.name}"/>
                </th>
                            ...
                            <c:forEach var="drug" items="${command.conceptDrugList}">
                                <c:choose>
                                    <c:when test="${not empty drug.dosageForm}">
                                        <li class="<c:if test="${drug.retired}">retired </c:if>"><c:out
                                                value="${drug.name}"/> <c:out value="${drug.strength}"/> <c:out
                                                value="${drug.dosageForm.name}"/></li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="<c:if test="${drug.retired}">retired </c:if>"><c:out
                                                value="${drug.name}"/> <c:out value="${drug.strength}"/></li>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                            <li>
                                <a href="${pageContext.request.contextPath}/admin/concepts/conceptDrug.list?conceptId=${command.concept.conceptId}"><openmrs:message
                                        code="Concept.manageDrugFormulary"/></a><br/>
                            </li>
                        </ul>
                    </td>
                </tr>
