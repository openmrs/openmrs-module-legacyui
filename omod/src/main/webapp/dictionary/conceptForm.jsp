...
<c:if test="${command.concept.conceptId != null}">
    <form class="inlineForm" id="jumpForm" action="" method="post">
        ...
        <a href="#previousConcept" id="previousConcept" valign="middle" accesskey=","
           onclick="return jumpToConcept('previous')"><openmrs:message code="general.previous"/></a>
        |
        <a href="concept.htm?conceptId=${command.concept.conceptId}" id="viewConcept" accesskey="v"><openmrs:message
                code="general.view"/></a>
        |
        <a href="conceptStats.form?conceptId=${command.concept.conceptId}" id="conceptStats" accesskey="s"
           valign="middle"><openmrs:message code="Concept.stats"/></a>
        |
        <a href="#nextConcept" id="nextConcept" valign="middle" accesskey="."
           onclick="return jumpToConcept('next')"><openmrs:message code="general.next"/></a>
    </form>
</c:if>
...
<c:if test="${command.concept.retired}">
    <div class="retiredMessage">
        ...
        <div><c:if test="${command.concept.retiredBy.personName != null}"> <openmrs:message code="general.byPerson"/> <c:out
                value="${command.concept.retiredBy.personName}"/> </c:if> <c:if
                test="${command.concept.dateRetired != null}"> <openmrs:message code="general.onDate"/> <openmrs:formatDate
                date="${command.concept.dateRetired}" type="long"/> </c:if> <c:if
                test="${command.concept.retireReason!=''}"> - <c:out value="${command.concept.retireReason}"/> </c:if></div>
        <openmrs:hasPrivilege privilege="Manage Concepts">
            <div>
                <form action="" method="post"><input type="submit" name="action"
                                                     value="<openmrs:message code="general.unretire"/>"/>
                </form>
            </div>
...
<openmrs:extensionPoint pointId="org.openmrs.dictionary.conceptFormHeader" type="html"
                        parameters="conceptId=${command.concept.conceptId}"/>
...
<form method="post" action="">
    ...
    <table id="conceptTable" cellpadding="2" cellspacing="0">
        ...
        <tr>
            ...
            <td style="padding-bottom: 0px; padding-left: 0px;">
                <c:forEach items="${command.locales}" var="loc" varStatus="varStatus">
                    <a id="${loc}Tab" class="tab ${loc}" href="#select${loc.displayName}"
                       onclick="return selectTab(this)">${loc.displayName}</a><c:if
                        test="${varStatus.last==false}"> | </c:if>
                </c:forEach>
            </td>
        </tr>
        <tr class="localeSpecific">
            <th valign="bottom">
                ...
                <img class="help_icon" src="${pageContext.request.contextPath}/images/help.gif" border="0"
                     title="<openmrs:message code="Concept.fullySpecified.help"/>"/>
            </th>
                        ...
                        <tr>
                            <td id="similarConceptsStart" valign="bottom">
                                <spring:bind path="command.namesByLocale[${loc}].name">
                                    <input type="text" name="${status.expression}" value="${status.value}"
                                           id="${status.expression}" class="largeWidth"
                                           onchange="setRadioValue(this, 'fullySpecPreferred[${loc}]')"/>
                                    <span class="error" id="duplicateConceptError" hidden="true"><openmrs:message
                                            code="Concept.error.fullySpecifiedName.notUnique"/></span>
                                </spring:bind>
                            </td>
							...
							<span
                                ...
                                <img class="help_icon" src="${pageContext.request.contextPath}/images/help.gif" border="0"
                                     <c:if test="${fn:length(command.synonymsByLocale[loc]) > 0}">style="visibility: visible"</c:if>
                                     <c:if test="${fn:length(command.synonymsByLocale[loc]) == 0}">style="visibility: hidden"</c:if>
                                     title="<openmrs:message code="Concept.name.localePreferred.help"/>"/>
                                <br/>
                                <spring:bind path="command.preferredNamesByLocale[${loc}]" ignoreNestedPath="true">
                                    <input id="fullySpecPreferred[${loc}]" type="radio" name="${status.expression}" value=<c:out
                                        value="${command.namesByLocale[loc].name}"/>"
        ...
        <tr class="localeSpecific">
            <th valign="top">
                <openmrs:message code="Concept.synonyms"/> <img class="help_icon_bottom"
                                                                src="${pageContext.request.contextPath}/images/help.gif"
                                                                border="0"
                                                                title="<openmrs:message code="Concept.synonyms.help"/>"/>
            </th>
            <c:forEach items="${command.locales}" var="loc">
                <td class="${loc}">
                    <c:forEach var="synonym" items="${command.synonymsByLocale[loc]}" varStatus="varStatus">
                        <spring:nestedPath path="command.synonymsByLocale[${loc}][${varStatus.index}]">
                            ...
                            <div>
                                ...
                                <spring:bind path="command.preferredNamesByLocale[${loc}]" ignoreNestedPath="true">
                                    <input id="possiblePrefName_[${loc}][${varStatus.index}]" type="radio"
                                           name="${status.expression}" value="<c:out value="${synonym.name}"/>"
                                           <c:if test="${synonym.localePreferred}">checked=checked</c:if> />
                                </spring:bind>
                                ...
                                <c:if test="${command.synonymsByLocale[loc][varStatus.index].conceptNameId == null}">
                                    <input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                                           onClick="removeSynonymElement(this, 'newConceptSynonym-${loc}', 'preferredContainer[${loc}]', '${fn:length(command.synonymsByLocale[loc])}', null)"/>
                                </c:if>
                                <c:if test="${command.synonymsByLocale[loc][varStatus.index].conceptNameId != null}">
                                    <input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                                           onClick="removeSynonymElement(this, 'newConceptSynonym-${loc}', 'preferredContainer[${loc}]', '${fn:length(command.synonymsByLocale[loc])}', 'synonymsByLocale[${loc}][${varStatus.index}].isVoided')"/>
                                </c:if>
                                <spring:bind path="name">
                                ...
                                <spring:bind path="voided">
                                    ...
                                    <input id="synonymsByLocale[${loc}][${varStatus.index}].isVoided" type="hidden"
                    ...
                    <div id="newConceptSynonym-${loc}" style="display: none">
                        ...
                        <input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                               onClick="removeSynonymElement(this, 'newConceptSynonym-${loc}', 'preferredContainer[${loc}]', null, null)"/>
                    </div>
        ...
        <tr class="localeSpecific">
            <th valign="top">
                <openmrs:message code="Concept.indexTerms"/> <img class="help_icon_bottom"
                                                                  src="${pageContext.request.contextPath}/images/help.gif"
                                                                  border="0"
                                                                  title="<openmrs:message code="Concept.indexTerms.help"/>"/>
            </th>
            <c:forEach items="${command.locales}" var="loc">
                <td class="${loc}">
                        ...
                        <spring:nestedPath path="command.indexTermsByLocale[${loc}][${varStatus.index}]">
                            ...
                            <div>
                                ...
                                <c:if test="${command.indexTermsByLocale[loc][varStatus.index].conceptNameId == null}">
                                    <input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                                           onClick="removeParentElement(this)"/>
                                </c:if>
                                <c:if test="${command.indexTermsByLocale[loc][varStatus.index].conceptNameId != null}">
                                    <input type="button" value='<openmrs:message code="general.remove"/>' class="smallButton"
                                           onClick="voidName(this, 'indexTermsByLocale[${loc}][${varStatus.index}].isVoided')"/>
                                </c:if>
                                <spring:bind path="name">
                                    <c:if test="${status.errorMessage != ''}">
                                        <span class="error">${status.errorMessage}</span>
                                ...
                                <spring:bind path="voided">
                                    <input type="hidden" name="_${status.expression}" value=""/>
                                    <input id="indexTermsByLocale[${loc}][${varStatus.index}].isVoided" type="hidden"
                                           name="${status.expression}" value="${status.value}"/>
                                </spring:bind>
                            </div>
                        </spring:nestedPath>
                    ...
                    <input type="button" id="addSearch" value='<openmrs:message code="Concept.indexTerm.add"/>'
                           class="smallButton"
                           onClick="cloneElement('newConceptIndexTerm-${loc}', ${fn:length(command.indexTermsByLocale[loc])}, 'indexTermsByLocale[${loc}]')"/>
                </td>
        ...
        <tr class="localeSpecific">
            <th>
                <openmrs:message code="Concept.shortName"/> <img class="help_icon_bottom"
                                                                 src="${pageContext.request.contextPath}/images/help.gif"
                                                                 border="0"
                                                                 title="<openmrs:message code="Concept.shortName.help"/>"/>
            </th>
            <c:forEach items="${command.locales}" var="loc">
                <td class="${loc}">
        ...
        <tr class="localeSpecific">
            <th>
                <openmrs:message code="general.description"/> <img class="help_icon_top"
                                                                   src="${pageContext.request.contextPath}/images/help.gif"
                                                                   border="0"
                                                                   title="<openmrs:message code="Concept.description.help"/>"/>
            </th>
            <c:forEach items="${command.locales}" var="loc">
        ...
        <tr>
            <th>
                <openmrs:message code="Concept.conceptClass"/> <img class="help_icon_bottom"
                                                                    src="${pageContext.request.contextPath}/images/help.gif"
                                                                    border="0"
                                                                    title="<openmrs:message code="Concept.conceptClass.help"/>"/>
            </th>
            <td valign="top">
                    ...
                    <select class="smallWidth" name="${status.expression}" id="conceptClass" onChange="changeClass(this);">
                        <c:forEach items="${classes}" var="cc">
                            <option value="${cc.conceptClassId}"
                                    <c:if test="${cc.conceptClassId == status.value}">selected="selected"</c:if>><c:out
                                    value="${cc.name}"/>
                            </option>
                        </c:forEach>
                    </select>
        ...
        <tr>
            <th valign="top">
                <openmrs:message code="Concept.set"/> <img class="help_icon"
                                                           src="${pageContext.request.contextPath}/images/help.gif"
                                                           border="0" title="<openmrs:message code="Concept.isSet.help"/>"/>
            </th>
            <td>
                <spring:bind path="command.concept.set">
                    <input type="hidden" name="_${status.expression}" value=""/>
                    <input type="checkbox" name="${status.expression}" id="conceptSet"
                           <c:if test="${status.value}">checked="checked"</c:if> onClick="changeSetStatus(this)"/>
                    <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                </spring:bind>
            </td>
        </tr>
        <tr id="conceptSetRow">
            ...
            <td valign="top">
                <spring:bind path="command.concept.conceptSets">
                    <input type="hidden" name="${status.expression}" id="conceptSets" size="40"
                           value='${command.setElements}'/>
                </spring:bind>
                <table cellpadding="0" cellspacing="0">
                    <tr>
                        <td valign="top">
                            <select class="largeWidth" size="6" id="conceptSetsNames" multiple="multiple"
                                    onkeyup="listKeyPress('conceptSetsNames', 'conceptSets', ' ', event);">
                                <c:forEach items="${command.concept.conceptSets}" var="set">
                                    <option value="${set.concept.conceptId}"><openmrs:format concept="${set.concept}"/>
                                        (${set.concept.conceptId})
                                    </option>
                                </c:forEach>
                            </select>
                        </td>
                        <td valign="top" class="buttons">
                            <span dojoType="ConceptSearch" widgetId="sSearch"></span><span dojoType="OpenmrsPopup"
                                                                                           searchWidget="sSearch"
                                                                                           searchTitle='<openmrs:message code="Concept.find"/>'
                                                                                           changeButtonValue='<openmrs:message code="general.add"/>'
                                                                                           showConceptIds="true"></span>
                            <input type="button" value="<openmrs:message code="general.remove"/>" class="smallButton"
                                   onClick="removeItem('conceptSetsNames', 'conceptSets', ' ');" style="display: block"/>
                            <input type="button" value="<openmrs:message code="general.move_up"/>" class="smallButton"
                                   onClick="moveUp('conceptSetsNames', 'conceptSets');" style="display: block"/>
                            <input type="button" value="<openmrs:message code="general.move_down"/>" class="smallButton"
                                   onClick="moveDown('conceptSetsNames', 'conceptSets');" style="display: block"/>
                        </td>
                    </tr>
                </table>
            </td>
        ...
        <tr>
            <th>
                <openmrs:message code="Concept.datatype"/> <img class="help_icon"
                                                                src="${pageContext.request.contextPath}/images/help.gif"
                                                                border="0"
                                                                title="<openmrs:message code="Concept.datatype.help"/>"/>
            </th>
            <td valign="top">
                <spring:bind path="command.concept.datatype">
                    <select class="smallWidth" name="${status.expression}" id="datatype" onChange="changeDatatype(this);"
                        ...
                        <c:forEach items="${datatypes}" var="cd">
                            <option value="${cd.conceptDatatypeId}"
                                    <c:if test="${cd.conceptDatatypeId == status.value}">selected="selected"</c:if>><c:out
                                    value="${cd.name}"/></option>
                        </c:forEach>
                    </select>
                    <c:if test="${dataTypeReadOnly == true && isBoolean != null && isBoolean == true}">
                        <input type="button" value="<openmrs:message code="Concept.boolean.add.answer"/>"
                               onclick="addAnswerToBooleanConcept('<openmrs:message
                                       code="Concept.boolean.confirm.add.answer"/>', '${command.concept.conceptId}')"
                ...
                <c:if test="${dataTypeReadOnly == true && isBoolean == null}">(<openmrs:message
                        code="Concept.datatype.readonly"/>)</c:if>
            </td>
        </tr>
        <tr id="codedDatatypeRow">
            <th valign="top">
                <openmrs:message code="Concept.answers"/>
                <img class="help_icon" id="tooltipCodedIcon" src="${pageContext.request.contextPath}/images/help.gif"
                     border="0" title="<openmrs:message code="Concept.datatype.coded.help"/>"/>
            </th>
            <td>
                <spring:bind path="command.concept.answers">
                    <input type="hidden" name="${status.expression}" id="answerIds" size="40"
                           value='<c:forEach items="${command.conceptAnswers}" var="answer">${answer.key} </c:forEach>'/>
                    <c:if test="${status.errorMessage != ''}"><span class="error">${status.errorMessage}</span></c:if>
                </spring:bind>
                <table cellspacing="0" cellpadding="0">
                    <tr>
                        <td valign="top">
                            <select class="largeWidth" size="6" id="answerNames" multiple="multiple"
                                    onKeyUp="listKeyPress('answerNames', 'answerIds', ' ', event)">
                                <c:forEach items="${command.conceptAnswers}" var="answer">
                                    <option value="<c:out value="${answer.key}" />"><c:out value="${answer.value}"/>
                        ...
                        <td valign="top" class="buttons">
                            <input type="button" value="<openmrs:message code="general.add"/>" class="smallButton"
                                   onClick="addAnswer();"/><br/>
                            <input type="button" value="<openmrs:message code="general.remove"/>" class="smallButton"
                                   onClick="removeItem('answerNames', 'answerIds', ' ');"/><br/>
                            <input type="button" value="<openmrs:message code="general.move_up"/>" class="smallButton"
                                   onClick="moveUp('answerNames', 'answerIds');" style="display: block"/>
                            <input type="button" value="<openmrs:message code="general.move_down"/>" class="smallButton"
                                   onClick="moveDown('answerNames', 'answerIds');" style="display: block"/>
                        </td>
                    </tr>
                </table>
            </td>
        ...
        <tr id="numericDatatypeRow">
            <th valign="top">
                ...
                <img class="help_icon" id="tooltipNumericIcon" src="${pageContext.request.contextPath}/images/help.gif"
                     border="0" title="<openmrs:message code="Concept.datatype.numeric.help"/>"/>
            </th>
            <td>
                <table border="0">
                    <tr>
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            <spring:bind path="command.allowDecimal">
                                <input type="hidden" name="_${status.expression}" value=""/>
                                <input type="checkbox" id="allow_decimal_checkbox" name="${status.expression}"
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            <spring:bind path="command.displayPrecision">
                                <input type="text" id="display_precision_textbox" name="${status.expression}"
        ...
        <tr id="complexDatatypeRow">
            <th valign="top">
                ...
                <img class="help_icon" id="tooltipComplexIcon" src="${pageContext.request.contextPath}/images/help.gif"
                     border="0" title="<openmrs:message code="Concept.datatype.complex.help"/>"/>
            </th>
            <td>
                <spring:bind path="command.handlerKey">
                    <select name="${status.expression}">
                        ...
                        <c:forEach var="handler" items="${handlers}">
                            <option value="${handler.key}"
                                    <c:if test="${handler.key == status.value}">selected="selected"</c:if>>
        ...
        <tr id="conceptMapRow">
            <th valign="top">
                <openmrs:message code="Concept.mappings"/> <img class="help_icon"
                                                                src="${pageContext.request.contextPath}/images/help.gif"
                                                                border="0"
                                                                title="<openmrs:message code="Concept.mappings.help"/>"/>
            </th>
            <td>
                            ...
                            <tr id="mapping-${mapStatus.index}">
                                <c:choose>
                                    <c:when test="${mapping.conceptMapId != null}">
                                                ...
                                                <select name="${status.expression}">
                                                    ...
                                                    <optgroup label="<openmrs:message code="Encounter.type.retired"/>">
                                                        ...
                                                        <option value="${record.conceptMapTypeId}"
                                                                <c:if test="${record.conceptMapTypeId == status.value}">selected="selected"</c:if> >
                                        ...
                                        <td <c:if test="${mapStatus.index % 2 == 0}">class='evenRow'</c:if>><c:out
                                                value="${mapping.conceptReferenceTerm.conceptSource.name}"/></td>
                                        <td <c:if test="${mapStatus.index % 2 == 0}">class='evenRow'</c:if>><c:out
                                                value="${mapping.conceptReferenceTerm.code}"/>
                                            <spring:bind path="conceptReferenceTerm">
                                                <input type="hidden" name="${status.expression}" value="${status.value}"/>
                                            </spring:bind>
                                        </td>
                                        <td <c:if test="${mapStatus.index % 2 == 0}">class='evenRow'</c:if>><c:out
                                                value="${mapping.conceptReferenceTerm.name}"/></td>
                                    </c:when>
                                    <c:otherwise>
                                        <td>
                                            <spring:bind path="conceptMapType">
                                        ...
                                        <td>
                                            <select id="term[${mapStatus.index}].source">
                                                <option value=""><openmrs:message
                                                        code="ConceptReferenceTerm.searchAllSources"/></option>
                                                <openmrs:forEachRecord name="conceptSource">
                                                    <option value="${record.conceptSourceId}"
                                                            <c:if test="${record.conceptSourceId == mapping.conceptReferenceTerm.conceptSource.conceptSourceId}">selected="selected"</c:if>>
                                                        <c:out value="${record.name}"/>
                                                    </option>
                                                </openmrs:forEachRecord>
                                            </select>
                                        ...
                                        <td>
                                            <spring:bind path="conceptReferenceTerm">
                                                <input type="text" id="term[${mapStatus.index}].code" name="term.code"
                                                       value="${mapping.conceptReferenceTerm.code}" size="25"/>
                                        ...
                                        <td <c:if test="${mapStatus.index % 2 == 0}">class='evenRow'</c:if>>
                                            <input type="text" id="term[${mapStatus.index}].name" size="25"
                                                   value="<c:out value="${mapping.conceptReferenceTerm.name}"/>"
                                                   readonly="readonly"/>
                                        </td>
                                    </c:otherwise>
                                </c:choose>
                        ...
                        <td colspan="3" valign="top" align="left">
                            <c:choose>
                                <c:when test="${sourceID != null}">
                                    <input id="addMapButton" type="button"
                                           value='<openmrs:message code="Concept.mapping.add"/>' class="smallButton"
        ...
        <c:forEach var="attrType" items="${ attributeTypes }">
            <c:if test="${ !attrType.retired }">
                <openmrs_tag:attributesForType attributeType="${ attrType }" customizable="${ command.concept }"
        ...
        <tr>
            <th>
                <openmrs:message code="Concept.version"/> <img class="help_icon_bottom"
                                                               src="${pageContext.request.contextPath}/images/help.gif"
                                                               border="0"
                                                               title="<openmrs:message code="Concept.version.help"/>"/>
            </th>
            <td>
    ...
    <div id="saveDeleteButtons" style="margin-top: 15px">
        <c:if test="${conceptsLocked != 'true'}">
            <input type="submit" name="action" value="<openmrs:message code="Concept.save"/>"
                   onMouseUp="removeHiddenRows()"/>
            <input type="submit" name="action" value="<openmrs:message code="Concept.saveAndContinue"/>"
                   onMouseUp="removeHiddenRows()"/>
            <input type="submit" name="action" value="<openmrs:message code="Concept.cancel"/>"
                   onMouseUp="removeHiddenRows()"/>
            <c:if test="${command.concept.conceptId != null}">
                <openmrs:hasPrivilege privilege="Delete Concepts">
                    &nbsp; &nbsp; &nbsp;
                    <input type="submit" name="action" value="<openmrs:message code="Concept.delete"/>"
                           onclick="return confirm('<openmrs:message code="Concept.confirmDelete"/>')"/>
                </openmrs:hasPrivilege>
            </c:if>
        </c:if>
    </div>
    ...
    <div id="create-new-term-dialog" title="<openmrs:message code="ConceptReferenceTerm.newTermForm"/>">
        ...
        <fieldset>
            ...
            <table cellpadding="3" cellspacing="3" align="center">
                <tr>
                    <th class="alignRight" valign="top"><openmrs:message code="ConceptReferenceTerm.code"/><span
                ...
                <tr>
                <tr>
                    <th class="alignRight" valign="top"><openmrs:message code="ConceptReferenceTerm.source"/><span
                            class="required">*</span></th>
                    <td valign="top">
...
<script type="text/javascript">
    ...
    $j(document).ready(function () {
        ...
        $j('#addAnswer').dialog({
            ...
            buttons: {
                '<openmrs:message code="general.add"/>': function () {
                    handleAddAnswer();
                },
