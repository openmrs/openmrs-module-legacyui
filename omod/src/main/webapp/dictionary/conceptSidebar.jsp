    ...
    <fieldset>
        ...
        <c:if test="${command.concept.conceptId!=null}">
            ...
            <c:if test="${fn:length(command.questionsAnswered) > 0}">
                <h4><openmrs:message code="dictionary.questionsAnswered"/></h4>
                <ul>
                    <c:forEach items="${command.questionsAnswered}" var="question">
                        <li><a href="concept.htm?conceptId=${question.conceptId}"><openmrs:format concept="${question}"/></a>
                        </li>
                    </c:forEach></ul>
            </c:if>

            <c:if test="${fn:length(command.containedInSets) > 0}">
                <h4><openmrs:message code="dictionary.containedInSets"/></h4>
                <ul>
                    <c:forEach items="${command.containedInSets}" var="set">
                        <li><a href="concept.htm?conceptId=${set.conceptSet.conceptId}"><openmrs:format
                                concept="${set.conceptSet}"/></a><br/></li>
                    </c:forEach></ul>
            </c:if>
            ...
            <openmrs:extensionPoint pointId="org.openmrs.concept.usage" type="html"
                                    requiredClass="org.openmrs.module.web.extension.ConceptUsageExtension">
