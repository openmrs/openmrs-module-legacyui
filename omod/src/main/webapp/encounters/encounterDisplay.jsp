...
<openmrs:extensionPoint pointId="org.openmrs.encounters.encounterListTop" type="html"
                        parameters="encounterId=${model.encounter.encounterId}">
        ...
        <div class="box" style="padding: 0px 0px 5px;"><openmrs:message code="${extension.content}"/>
            <c:if test="${extension.portletUrl != null}">
                <openmrs:portlet url="${extension.portletUrl}" moduleId="${extension.moduleId}" id="${extension.portletUrl}"
                                 encounterId="${model.encounter.encounterId}" parameters="allowEdits=true"/>
            </c:if>
        </div>
        <br/>
...
<div style="float: right">
    <openmrs:hasPrivilege privilege="Edit Encounters">
        <a href="javascript:void(0)"
           onClick="window.parent.location = '${pageContext.request.contextPath}/admin/encounters/encounter.form?encounterId=${model.encounter.encounterId}'; return false;">[
            <openmrs:message code="Encounter.edit"/> ]</a>
    </openmrs:hasPrivilege>
</div>
    ...
    <div id="page_${pageNumber}">
        <table class="encounterFormTable" cellpadding="0" cellspacing="0">
            <c:if test="${model.usePages}">
                <tr>
                    <td align="center" colspan="2" style="background-color: black; color: white;"><openmrs:message
                            code="Encounter.page" arguments="${pageNumber}"/></td>
                </tr>
            </c:if>
                            ...
                            <c:choose>
                                <c:when test="${fieldHolder.obsGrouping}">
                                    <table class="borderedTable">
                                        <tr>
                                            <th class="smallHeader obsGroupLabel"
                                                colspan="${fn:length(fieldHolder.groupMemberConcepts)}">
                                        ...
                                        <tr>
                                            <c:forEach var="conceptColumn" items="${fieldHolder.groupMemberConcepts}">
                                                <th class="smallHeader"><openmrs_tag:concept
                                                        conceptId="${conceptColumn.conceptId}"/></th>
                                            </c:forEach>
                                        </tr>
                                    ...
                                    <table class="encounterFormInnerTable" cellspacing="0" cellpadding="4">
                                        <c:forEach var="obs" items="${fieldHolder.obs}" varStatus="varStatusInner">
                                            <c:if test="${varStatusInner.first}">
                                                <tr <c:if
                                                        test="${varStatusInner.count > 1}"> class='${rowStatus == true ? "evenRow" : "oddRow" }' </c:if>>
                                                    <td class="encounterViewObsConcept"><openmrs_tag:concept
                                                            conceptId="${obs.concept.conceptId}"/>:
                                                    </td>
                                                    <td class="encounterViewObsAnswer">
                                                        <span class="encounterViewObsValue"><openmrs:format
                                                                obsValue="${obs}"/></span>
