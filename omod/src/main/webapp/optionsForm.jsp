...
<form method="post">
    ...
    <div id="optionsForm">
        <fieldset>
            <legend><openmrs:message code="options.default.legend"/></legend>
            <table>
                <tr>
                            ...
                            <select name="${status.expression}">
                                ...
                                <c:forEach items="${locations}" var="loc">
                                    <option value="${loc.locationId}"
                                            <c:if test="${loc.locationId == status.value}">selected</c:if>><c:out
                                            value="${loc.name}"/></option>
                                </c:forEach>
                            </select>
                        ...
                        <spring:bind path="opts.defaultLocale">
                            <select name="${status.expression}">
                                ...
                                <c:forEach items="${languages}" var="locale">
                                    <option value="${locale}"
                                            <c:if test="${locale == status.value}">selected</c:if>>${locale.displayName}</option>
                                </c:forEach>
                            </select>
                            <c:if test="${status.errorMessage != ''}">
                                <span class="error">${status.errorMessage}</span>
                ...
                <tr>
                    ...
                    <td>
                        <label for="${status.expression}">
                            <spring:bind path="opts.showRetiredMessage"> </label>
                        <input type="hidden" name="_${status.expression}" value="true"/>
                        <input type="checkbox" name="${status.expression}" value="true" id="${status.expression}"
        ...
        <fieldset>
            <legend><openmrs:message code="options.login.legend"/></legend>
            <table>
                <tr>
                ...
                <tr>
                    <td colspan="2"><openmrs:message code="options.login.secretQuestion.about"/></td>
                </tr>
                <tr>
                    <td><openmrs:message code="options.login.password.old"/></td>
                    <td>
        ...
        <fieldset>
            <legend><openmrs:message code="options.notify.legend"/></legend>
            <table>
                <tr>
                    <td><input type="radio" name="notification" value="internalOnly" id="internalOnly"
                               <c:if test="${opts.notification == 'internalOnly'}">checked</c:if> /></td>
                    <td><label for="internalOnly"><openmrs:message code="options.notify.internalOnly"/></label></td>
                </tr>
                <tr>
                    <td><input type="radio" name="notification" value="internal" id="internal"
                               <c:if test="${opts.notification == 'internal'}">checked</c:if> /></td>
                    <td><label for="internal"><openmrs:message code="options.notify.internal"/></label></td>
                </tr>
                <tr>
                    <td><input type="radio" name="notification" value="internalProtected" id="internalProtected"
                               <c:if test="${opts.notification == 'internalProtected'}">checked</c:if> /></td>
                    <td><label for="internalProtected"><openmrs:message code="options.notify.internalProtected"/></label>
                    </td>
                </tr>
                <tr>
                    <td><input type="radio" name="notification" value="email" id="email"
                               <c:if test="${opts.notification == 'email'}">checked</c:if> /></td>
        ...
        <openmrs:extensionPoint pointId="org.openmrs.userOptionExtension"
                                requiredClass="org.openmrs.module.web.extension.UserOptionExtension" type="html">
            <openmrs:hasPrivilege privilege="${extension.requiredPrivilege}">
                <c:catch var="ex">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <fieldset>
                                <legend>${extension.tabName}</legend>
                                <openmrs:portlet
                                        url="${extension.portletUrl}" id="${extension.tabId}"
                                        moduleId="${extension.moduleId}" parameters="${extension.portletParameters}"/>
                            </fieldset>
                        </c:otherwise>
                    </c:choose>
                </c:catch>
