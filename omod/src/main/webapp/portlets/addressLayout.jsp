        ...
        if (resultArray || obj.value == null || obj.value == "") {
            ...
            if (!overwrittenOnSubmit) {
                ...
                obj.form.onsubmit = function () {
                    alert('<openmrs:message code="fix.error.plain" javaScriptEscape="true"/>');
                    return false;
                };
                            ...
                            <td>
                                <spring:bind path="preferred">
                                    ...
                                    <input type="checkbox" name="${status.expression}"
                                           onclick="if (preferredBoxClick) preferredBoxClick(this)" value="true"
                                           alt="personAddress"
                        ...
                        <tr>
                            <c:forEach items="${line}" var="token" varStatus="tokenStatus">
                                <c:if test="${token.isToken == model.layoutTemplate.layoutToken}">
                                    ...
                                    <td
                                            <c:if test="${tokenStatus.last && tokenStatus.index < model.layoutTemplate.maxTokens}">colspan="${model.layoutTemplate.maxTokens - tokenStatus.index}"</c:if>>
                                        <c:catch var="exp">
                                            <spring:bind path="${token.codeName}">
                                                <c:if test="${token.codeName == 'endDate'}"><input type="hidden"
                                                                                                   name="_${status.expression}"></c:if>
                                                ...
                                                <c:if test="${model.isNew && empty elementValue && not empty model.layoutTemplate.elementDefaults && not empty model.layoutTemplate.elementDefaults[token.codeName]}">
                                                    <c:set var="elementValue"
                                                           value="${model.layoutTemplate.elementDefaults[token.codeName]}"/>
                                                </c:if>
                                                <input id="${status.expression}" type="text" name="${status.expression}"
                                                       value="${elementValue}" size="${token.displaySize}"
                                                       ...
                                                       onkeyup="<c:if
                                                               test='${model.layoutTemplate.elementRegex[token.codeName] !="" }'>validateFormat(this, '${model.layoutTemplate.elementRegex[token.codeName]}','${token.codeName}' )</c:if>"
                                                />
                                                ...
                                                <i name="formatMsg_${token.codeName}"
                                                   style="font-weight: normal; font-size: xx-small; color: red; display: none">
                                                    <c:choose>
                                                        <c:when test="${model.layoutTemplate.elementRegexFormats[token.codeName] != null }">
                                                            (<openmrs:message
                                                                code="general.format"/>: ${model.layoutTemplate.elementRegexFormats[token.codeName]})
                                                        </c:when>
                                                        <c:otherwise>
                                                            <openmrs:message code="general.invalid"/>&nbsp;<openmrs:message
                                                                code="general.format"/>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </i>
                    ...
                    <c:if test="${model.layoutShowExtended == 'true'}">
                        ...
                        <c:if test="${model.layoutHideVoidOption != 'true'}">
                                ...
                                <td>
                                    <spring:bind path="voided">
                                        ...
                                        <input type="checkbox" name="${status.expression}"
                                               ...
                                               onClick="toggleLayer('<spring:bind
                                                       path="personAddressId">voidReasonAddressRow-${status.value}</spring:bind>'); if (voidedBoxClicked) voidedBoxClicked(this); "
                                        />
                                    </spring:bind>
                                </td>
                            </tr>
                            <tr id="<spring:bind path="personAddressId">voidReasonAddressRow-${status.value}</spring:bind>"
                                style="
                                <spring:bind path="voided">
                                <c:if test="${status.value == false}">display: none;</c:if></spring:bind>">
