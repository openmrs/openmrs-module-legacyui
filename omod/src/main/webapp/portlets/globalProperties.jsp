    ...
    <table cellspacing="0" cellpadding="2">
        ...
        <c:forEach var="prop" items="${model.properties}" varStatus="status">
            ...
            <tr valign="top" <c:if test="${ind % 2 == 1}">style="background-color: #f0f0f0"</c:if>>
                ...
                <td>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <input type="text" size="80" id="gp_${ind}-${model.portletUUID}"
                                   onKeyUp="showDiv('gp_${ind}-${model.portletUUID}_actions')"/>
                        </c:otherwise>
                    </c:choose>
                </td>
                <td style="width: 10em">
            ...
            <c:if test="${model.hideDescription != 'true'}">
                <tr <c:if test="${ind % 2 == 1}">style="background-color: #f0f0f0"</c:if>>
                    <td colspan="3">
                        <small>
                            <c:if test="${not empty prop.description}"><i><c:out
                                    value="${prop.description}"/></i><br/></c:if>
                            &nbsp;
                        </small>
                    <td>
                </tr>
