...
<div id="displayEncounterPopup">
    ...
    <iframe id="displayEncounterPopupIframe" width="100%" height="100%" marginWidth="0" marginHeight="0" frameBorder="0"
...
<script type="text/javascript">
    $j(document).ready(function () {
        $j('#displayEncounterPopup').dialog({
            ...
            modal: true,
            open: function (a, b) {
                $j('#displayEncounterPopupLoading').show();
            }
        });
    });
...
<c:if test="${model.showPagination == 'true'}">
    <script type="text/javascript">
        $j(document).ready(function () {
            ...
            $j("#displayEncounterPopupIframe").load(function () {
...
<div id="portlet${model.portletUUID}">
    <div id="encounterPortlet">
        ...
        <c:if test="${enableFormEntryInEncounters && !model.hideFormEntry}">
            <openmrs:hasPrivilege privilege="Form Entry">
                <div id="formEntryDialog">
                    <openmrs:portlet url="personFormEntry" personId="${patient.personId}" id="encounterTabFormEntryPopup"
                                     parameters="showDefinedNumberOfEncounters=false|returnUrl=${model.formEntryReturnUrl}"/>
                </div>

                <button class="showFormEntryDialog" style="margin-left: 2em; margin-bottom: 0.5em"><openmrs:message
                        code="FormEntry.fillOutForm"/></button>
        ...
        <openmrs:hasPrivilege privilege="View Encounters">
            <div id="encounters">
                <div class="boxHeader${model.patientVariation}"><c:choose><c:when
                        test="${empty model.title}"><openmrs:message
                        code="Encounter.header"/></c:when><c:otherwise><openmrs:message code="${model.title}"
                                                                                        arguments="${model.num}"/></c:otherwise></c:choose></div>
                <div class="box${model.patientVariation}">
                    <div>
                        <table cellspacing="0" cellpadding="2" id="patientEncountersTable">
                            ...
                            <openmrs:forEachEncounter encounters="${model.patientEncounters}" sortBy="encounterDatetime"
                                                      descending="true" var="enc" num="${model.num}">
                                <tr class='${status.index % 2 == 0 ? "evenRow" : "oddRow"}'>
                                    ...
                                    <td class="encounterView" align="center">
                                        <c:if test="${showViewLink}">
                                            <c:set var="viewEncounterUrl"
                                                   value="${pageContext.request.contextPath}/admin/encounters/encounter.form?encounterId=${enc.encounterId}"/>
                                            <c:choose>
                                                <c:when test="${ model.formToViewUrlMap[enc.form] != null }">
                                                    <c:url var="viewEncounterUrl"
                                                           value="${model.formToViewUrlMap[enc.form]}">
                                                        <c:param name="encounterId" value="${enc.encounterId}"/>
                                                    </c:url>
                                                </c:when>
                                                <c:when test="${ model.formToEditUrlMap[enc.form] != null }">
                                                    <c:url var="viewEncounterUrl"
                                                           value="${model.formToEditUrlMap[enc.form]}">
                                                        <c:param name="encounterId" value="${enc.encounterId}"/>
                                                    </c:url>
                                                </c:when>
                                            </c:choose>
                                            <a href="${viewEncounterUrl}">
                                                <img src="${pageContext.request.contextPath}/images/file.gif"
                                                     title="<openmrs:message code="general.view"/>" border="0"/>
                                            </a>
                                        </c:if>
                                    </td>
                                    ...
                                    <td class="encounterVisit">
                                        <c:if test="${enc.visit != null}"><openmrs:format
                                                visitType="${enc.visit.visitType}"/></c:if>
                                    </td>
                                    <td class="encounterProvider"><openmrs:format
                                            encounterProviders="${enc.providersByRoles}"/></td>
