...
<%
    if (request.getParameter("patientGraphConcept") != null) {
        ...
        org.openmrs.api.context.Context.getAuthenticatedUser()
                .setUserProperty("patientGraphConcepts", userConcepts); // set this for the currently cached logged in user
        org.openmrs.User user = org.openmrs.api.context.Context.getUserService()
                .getUser(org.openmrs.api.context.Context.getAuthenticatedUser().getUserId());
        org.openmrs.api.context.Context.getUserService().setUserProperty(user, "patientGraphConcepts",
                userConcepts); // save it to the db for this user so it reappears when they log in again
    }
%>
                ...
                <tr>
                    <td>
                        <div id="conceptBox-${conceptIds}" style="text-align: center;">
                            ...
                            <a class="graphToggleLink" href="#conceptBox-${conceptIds}"><openmrs:message
                                    code="patientDashboard.graphs.hide"/></a>
                            <div class="conceptBox-body">
                                <div style="margin: 10px auto; height: 300px; width: 600px; align: center;" align="center"
                                     id="conceptGraphBox-${conceptIds}">
        ...
        <tr>
            <td>
                ...
                <openmrs_tag:conceptField formFieldName="concept" formFieldId="conceptId" excludeDatatypes="N/A"
                                          includeDatatypes="Numeric" onSelectFunction="onConceptSelect"/>
        ...
        $j.getJSON("patientGraphJson.form?patientId=<c:out value="${patient.patientId}" />&conceptId=${conceptIds}", function (json) {
            ...
            var plot = $j.plot($j('#conceptGraphBox-${conceptIds}'),
                [{
                    ...
                    yaxis: {
                        min: findMaxAndMin(json.data).min - 10,
                        max: findMaxAndMin(json.data).max + 10,
                        tickFormatter: function (v, axis) {
                            return v.toFixed(axis.tickDecimals) + " " + json.units
                        }
                    },
