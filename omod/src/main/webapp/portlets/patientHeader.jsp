...
<c:if test="${model.patientVariation == 'Exited'}">
    <div class="retiredMessage">
        <div>
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;
            <openmrs:message code="Patient.patientCareExitDate"/>: <openmrs:formatDate
                date="${model.patientReasonForExit.obsDatetime}"/>
            &nbsp;&nbsp;&nbsp;&nbsp;
            <openmrs:message code="Patient.patientCareExitReason"/>: <openmrs:format
                concept="${model.patientReasonForExit.valueCoded}"/>
        </div>
    </div>
</c:if>
...
<c:choose>
    <c:when test="${model.showPatientDashboardLink == true}">
        <a href="${pageContext.request.contextPath}/patientDashboard.form?patientId=${model.patient.id}"><c:out
                value="${model.patient.personName}"/></a></div>
    </c:when>
    <c:otherwise>
    ...
    <c:if test="${fn:length(model.patient.activeIdentifiers) > 0}">
        <c:forEach var="identifier" items="${model.patient.activeIdentifiers}"
			...
			<span class="patientHeaderPatientIdentifier"><span
                    id="patientHeaderPatientIdentifierType"><c:out
                    value="${identifier.identifierType.name}"/><openmrs:extensionPoint
    ...
    <tr class="patientHeaderGeneralInfoRow">
        ...
        <td id="patientHeaderOtherIdentifiers"><c:if
            ...
            <c:forEach var="identifier"
					...
					<span class="patientHeaderPatientIdentifier"><c:out
                            value="${identifier.identifierType.name}"/><openmrs:extensionPoint
                            pointId="org.openmrs.patientDashboard.afterPatientHeaderPatientIdentifierType"
                            type="html"
                            parameters="identifierLocation=${identifier.location.name}"/>:
                    ...
                    <tr>
                        ...
                        <c:forEach items="${programEnrollment.currentStates}"
                            ...
                            <c:if test="${ fn:contains(workflowsToShow, temp) }">
                                <td class="programEnrollmentBarData">|</td>
                                <td class="patientStateProgramWorkflowNameData"><c:out
                                        value="${patientState.state.programWorkflow.concept.name}"/>:
                                </td>
                                <th class="patientStateConceptNameHeader"><c:out
                                        value="${patientState.state.concept.name}"/></th>
                            </c:if>
                        </c:forEach>
                    </tr>
                </table>
                ...
                <table class="patientLastEncounterTable">
                    <tr class="patientLastEncounterRow">
                        ...
                        <th><c:forEach
                            ...
                            <c:out value="${lastEncounter.encounterType.name}"/> @ <c:out
                                value="${lastEncounter.location.name}"/>, <openmrs:formatDate
    ...
    <c:if test="${visitsEnabled }">
                    ...
                    <c:if test="${empty model.activeVisits}">
                        <div id="patientVisitsSubheader" class="box" style="margin-top: 2px">
                            <input type="button" value="<openmrs:message code="Visit.start"/>"
                                   onclick="window.location='<openmrs:contextPath/>/admin/visits/visit.form?patientId=<c:out
                                           value="${model.patient.patientId}"/>&startNow=true'"/>
                        </div>
                    </c:if>
        ...
        <openmrs:hasPrivilege privilege="View Visits, View Encounters">
            ...
            <div id="patientHeader-endvisit-dialog" title="<openmrs:message code="Visit.end"/>">
                <form:form action="${pageContext.request.contextPath}/admin/visits/endVisit.form" method="post"
                    ...
                    <table cellpadding="3" cellspacing="3" align="center">
                        ...
                        <tr>
                            <td colspan="2" style="text-align: center">
                                ...
                                &nbsp;
                                <input id="patientHeader-close-endvisit-dialog" type="button"
                ...
                <div id="patientVisitsSubheader" class="box" style="margin-top: 2px">
                    ...
                    <openmrs:hasPrivilege privilege="Edit Visits">
                        <input type="button" value="<openmrs:message code="Visit.edit"/>"
                               onclick="window.location='
                                   <openmrs:contextPath/>/admin/visits/visit.form?visitId=${ visit.visitId }&patientId=<c:out
                                       value="${model.patient.patientId}"/>'"/>
                        <input type="button" value="<openmrs:message code="Visit.end"/>"
                               onclick="patientHeaderEndVisit('${visit.visitId}', '<openmrs:formatDate
                                       date="${visit.stopDatetime}" format="dd/MM/yyyy HH:mm"/>');"/>
                    </openmrs:hasPrivilege>
                    <br/>&nbsp;
                    ...
                    <c:forEach var="encounter" items="${visit.nonVoidedEncounters}" varStatus="status">
                        <c:set var="viewEncounterUrl"
                               value="${pageContext.request.contextPath}/admin/encounters/encounter.form?encounterId=${encounter.encounterId}"/>
                        <c:choose>
                        ...
                        <a href="${viewEncounterUrl}">
                            <openmrs:format encounterType="${encounter.encounterType}"/></a><c:if
