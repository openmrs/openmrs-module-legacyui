...
<div id="editPatientProgramPopup"
     style="position: absolute; background-color: #e0e0e0; z-index: 5; padding: 10px; border: 1px black dashed; display: none">
    <table>
        <tr>
        ...
        <tr>
            ...
            <td>
                <select name="locationId" id="programLocationElement">
                    ...
                    <c:forEach var="location" items="${model.locations}">
                        <c:if test="${location.retired}">
                            <option value="${location.locationId}"><c:out value="${location.displayString}"/>
                                (<openmrs:message code="general.retired"/>)
                            </option>
                        </c:if>
                    </c:forEach>
                </select>
        ...
        <tr>
            ...
            <td><span id="createdByElement"></span>&nbsp;<openmrs:message code="general.onDate"/>&nbsp;<span
                    id="dateCreatedElement"></span></td>
        </tr>
        <tr id="changedByTR" style="display:none;">
            <td><openmrs:message code="general.changedBy"/>:</td>
            <td><span id="changedByElement"></span>&nbsp;<openmrs:message code="general.onDate"/>&nbsp;<span
                    id="dateChangedElement"></span></td>
        </tr>
    </table>
    <table width="400">
        <tr>
            ...
            <td align="center">
                <input type="button" value="<openmrs:message code="general.cancel"/>"
                       onClick="currentProgramBeingEdited = null; hideLayer('editPatientProgramPopup')"/>
            </td>
            <td align="center">
                <!-- <input type="button" value="<openmrs:message code="general.delete"/>" onClick="handleDeleteProgram()" />	 -->
                <span style="position: relative">
				    <input type="button" id="deletePatientProgramButton" value="<openmrs:message code="general.delete"/>"
                           onClick="showDiv('deletePatientProgramDiv')"/>
					<div id="deletePatientProgramDiv"
                         style="position: absolute; padding: 1em; bottom: -5px; left: 0px; z-index: 9; width: 250px; border: 1px black solid; background-color: #E0E0F0; display: none">
					    <openmrs:message code="general.voidReasonQuestion"/>:&nbsp;&nbsp;<input type="text"
                                                                                                id="voidReason_PatientProgram"
                                                                                                size="15"/>
						<br/><br/>
						<div align="center">
							<input type="button" value="<openmrs:message code="general.delete"/>"
                                   onclick="handleDeleteProgram()"/>
							&nbsp; &nbsp; &nbsp;
							<input type="button" value="<openmrs:message code="general.cancel" />"
                                   onClick="hideDiv('deletePatientProgramDiv')"/>
						</div>
					</div>
				</span>
            </td>
...
<div id="editWorkflowPopup"
    ...
    <select id="changeToState">
        <option value=""><openmrs:message code="general.loading"/></option>
    </select>
    on
    ...
    <input type="button" value="<openmrs:message code="general.cancel"/>"
           onClick="currentWorkflowBeingEdited = null; hideLayer('editWorkflowPopup')"/>
</div>
                                ...
                                <c:if test="${!workflow.retired}">
                                    <tr>
                                        <td valign="top">
                                            ...
                                            <small><openmrs:format concept="${workflow.concept}"
                                                                   caseConversion="global"/>:</small>
                                            <br/>
                                            ...
                                            <c:choose>
                                                <c:when test="${not empty stateId}">
                                                    <b <c:if test="${retired}"> class="voided"} </c:if> ><openmrs:format
                                                            conceptId="${stateId}" caseConversion="global"/></b>
                                            ...
                                            <c:if test="${program.dateCompleted == null}">
                                                <a href="javascript:showEditWorkflowPopup('<openmrs:concept conceptId="${workflow.concept.conceptId}" nameVar="n" var="v" numericVar="nv"><c:out value="${n.name}"/></openmrs:concept>', ${program.patientProgramId}, ${workflow.programWorkflowId})">[<openmrs:message
                                                        code="general.edit"/>]</a>
                                            </c:if>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
    $j(document).ready(function () {
        ...
        $j('#enrollInProgramDialog').dialog({
            ...
            zIndex: 100,
            buttons: {
                '<openmrs:message code="Program.enrollButton"/>': function () {
...
<div id="enrollInProgramDialog" style="display:none;">
    ...
    <form id="enrollForm" name="enrollForm" method="post"
          action="${pageContext.request.contextPath}/admin/programs/patientProgram.form">
        ...
        <input type="hidden" name="returnPage"
               value="${pageContext.request.contextPath}/patientDashboard.form?patientId=<c:out value="${model.patientId}" />"/>
        <table style="margin: 0px 0px 1em 2em;">
                    ...
                    <select id="programSelector" name="programId">
                        ...
                        <c:forEach var="program" items="${model.programs}">
                            <c:if test="${!program.retired}">
                                <option id="programOption${program.programId}" value="${program.programId}"><openmrs:format
            ...
            <tr id="initialStateSection" style="display:none;">
                <td valign="top"><openmrs:message code="Program.initialStates"/><br/>(<openmrs:message
                        code="general.optional"/>)
                </td>
                <td>
                                            ...
                                            <select name="initialState.${wf.programWorkflowId}">
                                                ...
                                                <c:forEach items="${wf.sortedStates}" var="wfState">
                                                    <c:if test="${wfState.initial && !wfState.retired}">
                                                        <option value="${wfState.programWorkflowStateId}"><openmrs:format
                                                                concept="${wfState.concept}"/></option>
                                                    </c:if>
                                                </c:forEach>
                                                <c:forEach items="${wf.sortedStates}" var="wfState">
                                                    <c:if test="${wfState.initial && wfState.retired}">
                                                        <option class="retired" value="${wfState.programWorkflowStateId}">
                                                            <openmrs:format concept="${wfState.concept}"/></option>
                                                    </c:if>
                                                </c:forEach>
                                            </select>
                                        </td>
