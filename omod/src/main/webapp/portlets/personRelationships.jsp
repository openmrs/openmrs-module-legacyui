...
<script type="text/javascript">
    $j(document).ready(function () {
        $j('#addRelationship').dialog({
            ...
            zIndex: 100,
            buttons: {
                '<openmrs:message code="general.save"/>': function () {
        ...
        $j('#voidRelationship').dialog({
            ...
            zIndex: 100,
            buttons: {
                '<openmrs:message code="general.remove"/>': function () {
                    handleVoidRelationship();
                },
                '<openmrs:message code="general.cancel"/>': function () {
                    $j(this).dialog("close");
                }
...
<div id="patientRelationshipPortlet">
    ...
    <div id="addRelationship">
                    ...
                    <c:choose>
                        <c:when test="${relType.aIsToB == relType.bIsToA}">
                            <td style="text-align: center; white-space: nowrap" align="center" colspan="3">
                                <a href="javascript:handlePickRelType('${relType.relationshipTypeId}', '${relType.aIsToB}')"><c:out
                                        value="${relType.aIsToB}"/></a>
                            </td>
                        </c:when>
                        <c:otherwise>
                            <td style="text-align: right; white-space: nowrap; width: 49%">
                                <a onclick="handlePickRelType('${relType.relationshipTypeId}', '${relType.aIsToB}')"><c:out
                                        value="${relType.aIsToB}"/></a>
                            </td>
                            <td width="2%">:</td>
                            <td style="text-align: left; white-space: nowrap; width: 49%">
                                <a onclick="handlePickRelType('${relType.relationshipTypeId}::reverse', '${relType.bIsToA}')"><c:out
                                        value="${relType.bIsToA}"/></a>
                            </td>
                        </c:otherwise>
                    </c:choose>
                </tr>
        ...
        <span id="add_rel_details" style="display: none">
			...
			<openmrs_tag:personField formFieldName="add_rel_target" formFieldId="add_rel_target_id"
                                     displayFieldId="add_rel_display_id" searchLabel="Find a Person" canAddNewPerson="true"/>
			<br/>
