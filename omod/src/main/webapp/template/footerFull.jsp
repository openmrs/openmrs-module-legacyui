		...
		<div id="footerInner">
			...
			<span id="localeOptions">
				<%  //removes last instance of lang= from querystring and encodes the url to avoid xml problems
					String qs = org.apache.commons.lang3.StringEscapeUtils.escapeXml(request.getQueryString());
					if (qs == null)
						qs = "";
            ...
            <c:forEach items="${locales}" var="loc" varStatus="status">
                <c:set var="locDisplayName" value="${loc.displayName}"/>
                <c:if test="${status.index != 0}">| </c:if>

                <c:choose>
                <c:when test="${locale eq loc}">
                ${locDisplayName}
                </c:when>
                <c:otherwise>
