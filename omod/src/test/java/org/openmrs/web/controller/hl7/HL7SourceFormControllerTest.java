 ...
 * on GET-time JSP rendering. The form JSP carries an
 * {@code <openmrs:require privilege="Update HL7 Source"/>} guard, but that tag only fires when the
 * JSP is rendered. A successful POST is dispatched through {@code onSubmit} and returned via a
 * {@code RedirectView}, so the JSP never renders and the privilege check is bypassed unless
 * something else enforces it.
 */
public class HL7SourceFormControllerTest extends BaseModuleWebContextSensitiveTest {
	...
	/**
	 * Dispatches the request through {@link #legacyUiUrlMapping} so that any interceptors
	 * registered on the mapping (in particular, {@code AuthorizationHandlerInterceptor}) actually
	 * run before the handler. {@link WebTestHelper#handle} resolves the handler but skips
	 * {@code preHandle}, which means it does not exercise the interceptor and cannot prove the
	 * authorization wiring is correct.
	 */
	private void dispatchThroughInterceptors(MockHttpServletRequest request) throws Exception {
		...
		if (chain != null && chain.getInterceptors() != null) {
	...
	/**
	 * A user with no HL7 privileges must not be able to mutate an HL7 source by POSTing to
	 * {@code /admin/hl7/hl7Source.form}. If this test fails, the vulnerability described in the
	 * report is exploitable on master.
	 */
	@Test
	public void onSubmit_shouldRejectUnprivilegedUserAttemptingToSaveHL7Source() throws Exception {
		...
		assertNotNull("standardTestDataset should provide hl7_source #1", originalSource);
		...
		assertEquals("An authenticated user without 'Update HL7 Source' must not be able to modify an HL7 source via POST",
		    originalName, after.getName());
		assertEquals("An authenticated user without 'Update HL7 Source' must not be able to modify an HL7 source via POST",
		    originalDescription, after.getDescription());
	}
	
	/**
	 * Models the reporter's actual scenario: a user with read access to HL7 sources (
	 * {@code Get HL7 Source}) but not write access ({@code Update HL7
	 * Source}). With this privilege set, {@code formBackingObject} succeeds (because
	 * {@code getHL7Source} only requires {@code Get HL7 Source}), so the dispatch reaches
	 * {@code onSubmit}; only the service-layer {@code @Authorized} on {@code saveHL7Source} stands
	 * between the attacker and the database write.
	 */
	@Test
	public void onSubmit_shouldRejectUserWithReadOnlyHL7PrivilegeAttemptingToSave() throws Exception {
		...
		java.util.List<java.util.List<Object>> rows = Context.getAdministrationService().executeSQL(
		    "select name from hl7_source where hl7_source_id = " + EXISTING_HL7_SOURCE_ID, true);
		...
		assertEquals(
		    ...
		    originalName, rows.get(0).get(0));
	}
	
	/**
	 * Drift-proof variant: same exploit attempt as
	 * {@link #onSubmit_shouldRejectUserWithReadOnlyHL7PrivilegeAttemptingToSave()} but the test
	 * explicitly calls {@link Context#flushSession()} after the dispatch, simulating any future (or
	 * already-deployed) configuration in which a dirty Hibernate session ends up flushed before
	 * request close.
	 * <p>
	 * Without {@link AuthorizationHandlerInterceptor} this fails: the data binder mutates the
	 * managed entity in the L1 cache, then our explicit flush pushes that change to the database.
	 * With the interceptor wired, {@code preHandle} rejects the request before
	 * {@code formBackingObject} runs, so no entity is loaded and there is nothing dirty to flush.
	 * This is the test that proves the interceptor closes the architectural defect, not just the
	 * current happy-path behaviour.
	 */
	@Test
	public void onSubmit_shouldRejectReadOnlyUserEvenWhenSessionIsExplicitlyFlushed() throws Exception {
		...
		java.util.List<java.util.List<Object>> rows = Context.getAdministrationService().executeSQL(
		    "select name from hl7_source where hl7_source_id = " + EXISTING_HL7_SOURCE_ID, true);
		assertEquals("expected exactly one row", 1, rows.size());
		assertEquals("Even with an explicit Hibernate flush after the request, an unprivileged caller must not be "
		        ...
		        + "did not match the URL.", originalName, rows.get(0).get(0));
	}
	
	/**
	 * A user with no HL7 privileges must not be able to purge an HL7 source by POSTing the
	 * {@code purge} action.
	 */
	@Test
	public void onSubmit_shouldRejectUnprivilegedUserAttemptingToPurgeHL7Source() throws Exception {
		...
		HL7Source after = Context.getHL7Service().getHL7Source(EXISTING_HL7_SOURCE_ID);
		assertNotNull("An authenticated user without 'Purge HL7 Source' must not be able to purge an HL7 source via POST",
		    after);
	}
}
