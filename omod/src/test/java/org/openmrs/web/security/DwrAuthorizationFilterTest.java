 ...
 * {@code config.xml}, and per-request authorization. These tests do not depend on the DWR servlet,
 * on the DwrFilter rewrite, or on any specific DWR script class beyond the ones already declared in
...
public class DwrAuthorizationFilterTest extends BaseModuleWebContextSensitiveTest {
	...
	public void init_everyDeclaredDwrMethodShouldBeAnnotated() {
		...
		assertTrue("All DWR methods must carry @RequirePrivilege. Unannotated: " + filter.getUnannotatedMethods().keySet(),
	...
	public void doFilter_shouldAllowAuthenticatedCallerWithRequiredPrivilege() throws Exception {
		...
		assertTrue("admin holds all migration privileges and must be passed to the chain", chain.invoked);
	}
	...
	public void doFilter_shouldPassStaticAssetsThroughWithoutAuth() throws Exception {
		...
		assertTrue("Static DWR assets (engine.js, util.js, generated interface JS) must be reachable without auth",
		    chain.invoked);
	}
	...
	public void doFilter_unknownScriptMethod_shouldRejectEvenAdmin() throws Exception {
		...
		assertEquals("Calls to {Script}.{method} pairs not declared in config.xml must be rejected even for admin", 403,
		    response.getStatus());
		assertFalse(chain.invoked);
	}
